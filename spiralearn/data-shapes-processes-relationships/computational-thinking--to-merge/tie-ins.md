...
**These are findings, not a work order.** Every cell states what _is_ the case,
in the indicative. If a cell reads as an instruction, that is a defect in this
file — rewrite the cell, do not follow it. Nothing in the F&V curriculum is
...
| Site                                      | What it already does                                                                                                                                                                                                | The finding, stated flat                                                                                                                                                                        |
| ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| `ontology.md:936-946`                     | Ch2's data-flow loop already models the user as a data-transforming system — data in through the eyes, transformed, back through a resolve event                                                                    | This is modelling computation with a human target — and, once the program is in a real user's hands, meaningful computation. It is the counterexample to any ruling that V's practice is not CT |
| `ontology.md:307-308`, `:697`             | "Computational thinking — F's practice. Traverses artifact ↔ NM ↔ CS/theory"                                                                                                                                        | Correct as written. The **definition** of computational thinking is the only thing the ontology never gives                                                                                     |
| `ontology.md:320-325`                     | Fixes F&V's window at user ↔ artifact ↔ NM, with CS/theory named as outside it                                                                                                                                      | Correct as written. Welcome to Algorithms is a sibling window — artifact ↔ NM ↔ CS/theory — not a correction to this one                                                                        |
| `ontology.md:1428`                        | The Ch2 simulated user as "Role-1-flavored practice apparatus, with its fallibility ceiling"                                                                                                                        | This is an **observability prosthesis for lens 6**, and its fallibility ceiling is a measurable property of the instrument rather than an isolated caveat                                       |
...
| Site                                                     | The conflict                                                                                                                                                    | The finding, stated flat                                                                                                                                                              |
| -------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ontology.md:1478`, `chapters.md:266`, `chapters.md:283` | Literal "domain-agnostic". `:266` reads "**largely** domain-agnostic… **the same NM skills transfer to any domain**" — a hedge _and_ an explicit transfer claim | Two senses are run together here: subject-matter-agnostic and discipline-situated. The transfer claim is broader than transfer of _practice_ supports                                 |
| `README.md:486`                                          | _Paraphrase_, not the token: "We focus on the machine, not on any specific domain the machine might be used"                                                    | The same two senses, and this site carries no literal "domain-agnostic" string                                                                                                        |
| `chapters.md:238`, `:240`, `README.md:19-20`             | Learning-to-program → programming-to-learn as a **single pivot**, with Ch4 as "the first pivot" and F&V a "prerequisite for" curricula requiring CT             | Three sites, not one. All three read it as a single pivot; it is repeated micro-transitions, and the prerequisite relation is the overlap of two chain windows                        |
| `ontology.md:873-877`                                    | Affordance defined in build terms only — "Lisp affords macros; Haskell affords laziness"                                                                        | Only one relatum is present: a language affords moves, but not **thoughts**. Second homes at `README.md:329-337`, `chapters.md:200-201`, `guide.authors.md:206`, all build-terms only |
| `ontology.md:1011-1012` → see `chapters.md:1011-1012`    | The control panel is "hold, transform, branch, repeat"                                                                                                          | Its I/O bookends are absent: the panel is **receive → hold → transform → branch → repeat → emit**. I/O is where the target system touches the artifact                                |
| `ontology.md:995`                                        | Lens 7, systemic impacts, marked "Practiced"                                                                                                                    | Under the legibility criterion it is reach, not practice — rhetorical and ethical reasoning that CT informs. Symmetric with lenses 1–3, no special pleading                           |
...
| Site                                                                                                                                                               | Finding                                                                                                                                   | The finding, stated flat                                                                                                                                                                                                                                                  |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| `ontology.md` §9 scope column                                                                                                                                      | Lens deferral currently reads as editorial scope-discipline                                                                               | The legibility criterion is its unstated reason, which makes lens 3 contingently rather than permanently deferred                                                                                                                                                         |
| `ontology.md:295-297`, `:307-308`, `:320-325`                                                                                                                      | F's range, artifact ↔ NM ↔ CS/theory, and the course window                                                                               | **Refined, not corrected.** The F-axis in `the-chain.svg` is the same range at higher resolution — code, the computational thoughts, pseudocode and formal notation are four positions §3 leaves implicit between its chain-points. §5's ATT tiers map onto the same axis |
| `ontology.md` §9 "the ladder as a ring"                                                                                                                            | A ladder read as a loop, held as "a coupling, not a precedence" that "adds no rung" and is "deliberately absent from learner-facing copy" | **Precedent, reused.** The chain wraps the same way — code studied formally, theory applied to physical computation — and is handled in that register rather than a new convention                                                                                        |
| `ontology.md:332`                                                                                                                                                  | "affordance-discovery cycles — plural deliberately; different grains of cycle"                                                            | The **spiral-turn grain** is unenumerated, though the plural already licenses it                                                                                                                                                                                          |
