...
Seven generators producing the domain fields for control-flow trace events across
three categories — `conditional` (if / ternary), `loop` (while / doWhile / for /
forOf), and `jump` (break / continue). See [DOCS.md](./DOCS.md) for design
decisions.
...
- `create-iteration-event.ts` — loop iteration start (`loop`; with forOf-specific
  fields)
...
- `create-for-initialize-event.ts` — for-loop setup phase (`loop`, event `setup`,
  kind `for`)
...
dispatcher stamps the base fields (step, semantics, nodePath, type, loc, source).
