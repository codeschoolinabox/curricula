...
Lives in `trace/tests/profiles/`. See `trace/tests/profiles/README.md` for the
full catalog.
...
All T7 tests use a **real Worker, real SharedArrayBuffer, real
`Atomics.wait()`**. No mocking.
...
- Run serially (`browser.instances: 1` or equivalent) — SAB contention causes
  random timeouts
- Requires cross-origin isolation: `COOP: same-origin` + `COEP: require-corp`
  headers for SharedArrayBuffer
- Symptom of SAB contention: timeout failures that don't reproduce consistently
  → increase `testTimeout`, reduce parallelism

---

## Related

- `trace/DOCS.md` — vocabulary, architecture axes, test taxonomy, key design
  decisions
