...
describe('createInterceptGenerator outcome classification (browser)', () => {
	...
	describe('outcome ↔ ok consistency matrix', () => {
		...
		// `it.each` rows can't `await` directly (top-level await in array
		// literals isn't allowed inside non-async describe). Resolve the
		// formatted-source rows lazily by passing a `codeLoader: () => Promise<string>`
		// the test invokes inside its async body.
		it.each([
			{
			...
			},
			{
				...
				codeLoader: () => format('while (true) { let x = 1; }\n'),
				options: { seconds: 0.1 },
			...
			},
			{
				...
				codeLoader: () =>
					format('for (let i = 0; i < 1000; i = i + 1) { let x = 1; }\n'),
			...
			'$label',
			async ({ codeLoader, options, expectedOutcome, expectedOk }) => {
				const code = await codeLoader();
	...
	describe('events are pure — no synthetic termination markers', () => {
		it('cancel: events contain only live events, not a trailing cancel marker', async () => {
			const gen = createInterceptGenerator('console.log(1);\n');
			await gen.next();
			gen.cancel();
			const result = await gen;
		...
		it('fail: events contain only live events, not a trailing fail marker', async () => {
			const gen = createInterceptGenerator('console.log(1);\n');
			await gen.next();
			gen.fail('test');
			const result = await gen;
