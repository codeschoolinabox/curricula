...
flowchart TD
    ...
    Col --> Label["station name + status label<br/>[data-orchestrator-station-status-label]"]
    Col --> Select["&lt;select&gt; over roster[station]<br/>sentinel first (disabled+hidden);<br/>disabled iff roster empty;<br/>value = rosters activeLens ? activeLens : ''"]
...
- **Disabling is roster-driven only** — `roster[station].length === 0`. Status
  never disables: lens availability is never gated, so a `barred` station with a
  staffed roster stays interactive.
...
- **Visual treatment is minimal by design** (Phase-1 choice): statuses render as
  data attributes + a plain-text label; CSS polish rides later cosmetic
  redirects. Tests anchor on attributes, never label text.
