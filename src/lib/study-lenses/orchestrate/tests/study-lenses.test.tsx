...
describe('<StudyLenses> — Cycle 3 D2 wire the quiz button to socratizing', () => {
	// Interface beat (ZOMBIES 'I') — the `generating` transient is intentionally
	// NOT tested: analyzeMicroDecisions is synchronous, so the true→false flip
	// batches into a single React commit and never paints. There is no reliable
	// DOM state to assert; the busy affordance is exercised at the D1 leaf, where
	// `generating` is a direct prop.
	describe('Zero/One — the quiz button mounts inside the region (both modes)', () => {
		it('renders the quiz button inside the region in editor mode', () => {
			const { container } = render(<StudyLenses snippet="let x = 5;" />);
			expect(
				container.querySelector(
					'[data-orchestrator-omnipresent-region] [data-orchestrator-quiz]',
				),
			).not.toBeNull();
		});

		it('renders the quiz button inside the region in lens mode too', () => {
			const { container } = render(
				<StudyLenses snippet="let x = 5;" lens="debug-props" />,
			);
			expect(
				container.querySelector(
					'[data-orchestrator-omnipresent-region] [data-orchestrator-quiz]',
		...
		it('keeps exactly one omnipresent-region landmark after adding the quiz button', () => {
			const { container } = render(<StudyLenses snippet="let x = 5;" />);
			expect(
		...
		it('keeps exactly one omnipresent-region landmark in lens mode', () => {
			const { container } = render(
				<StudyLenses snippet="let x = 5;" lens="debug-props" />,
			);
			expect(
	...
	describe('Zero — no questions before the first click', () => {
		it('renders no questions before the quiz button is clicked', () => {
			const { container } = render(<StudyLenses snippet="let x = 5;" />);
			expect(
	...
	describe('One — clicking Quiz on a parseable snippet populates the surface', () => {
		it('clicking Quiz on a parseable snippet populates the question surface', () => {
			const { container } = render(<StudyLenses snippet="let x = 5;" />);
			fireEvent.click(container.querySelector('[data-orchestrator-quiz]')!);
			expect(
				container.querySelectorAll('[data-orchestrator-quiz-question]').length,
			).toBeGreaterThan(0);
		});

		it('clicking Quiz in lens mode also populates the question surface', () => {
			const { container } = render(
				<StudyLenses snippet="let x = 5;" lens="debug-props" />,
			);
			fireEvent.click(container.querySelector('[data-orchestrator-quiz]')!);
	...
	describe('Boundary — clicking Quiz on a parse failure leaves the surface empty', () => {
		it('clicking Quiz on FAIL_AT_PARSE leaves the surface empty', () => {
			const { container } = render(<StudyLenses snippet="FAIL_AT_PARSE" />);
			fireEvent.click(container.querySelector('[data-orchestrator-quiz]')!);
			expect(
				container.querySelectorAll('[data-orchestrator-quiz-question]'),
