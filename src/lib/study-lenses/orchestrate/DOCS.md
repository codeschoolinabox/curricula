...
1. **Compose** — two cadences. At mount, sync and loud: the default lens roster
   ...
   cascade re-resolves per lens name over the two layers this region sees — the
   configs prop on top, the learner's tweaks always final. Input: the host
...
3. **Render** (mechanical) — the six-phase panel renders the embodiment; the
   ...
   through its honor path; recommendations render here, ranked, through the
   mask. Input: embodiment + verdicts + composed study configuration. Output:
   the rendered study environment.
...
flowchart TD
    ...
    VER["level verdicts + fit marks<br/>(one memoized validate per settle + level)"]
    ...
    VER -->|"mark · annotate · mask = verdict × strict posture"| SUR
...
- **Why memoization is here.** Three consumers — selector, gutter, mask —
  ...
  facts.
- **Why the mask derives at render.** Mask state follows the settled verdict, so
  it never flaps mid-keystroke; deriving it any earlier would couple enforcement
  to typing.
...
- **Selector and strict session state's internal home** — how the session's
  choices are held and how they meet the internal event bus belongs to the
  level-UI and event-bus internals, documented there.
