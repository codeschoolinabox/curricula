/**
 * @file CodeMirror decoration glue for the `diff` scaffold's WRITE half. Maps
 * the pure per-line verdict from `./diff-lines.ts` onto CodeMirror line
 * decorations. The READ half of the diff PAIR lives in
 * `./build-read-marker-field.ts` (one cohesive feature, sharing `diffLines`,
 ...
 * round trip. The decoration MAPPING — which lines land in the set — is
 * unit-tested by reading the field value off an `EditorState` (pure state,
 * jsdom-feasible); whether the highlight visually renders is a browser-gate
 * assertion (jsdom has no CodeMirror layout).
 */
 ...
 *
 * @param solution - the original source (`embodiment.facts.source.value`) to
 *   diff against.
...
export default function buildWriteDiffField(
	...
	return StateField.define<DecorationSet>({
		...
		update(value, transaction) {
			// Full recompute on any doc change — cheap (O(lines)) and keeps
			// positions correct after edits that shift lines. Non-doc
			// transactions (selection, focus) reuse the prior set.
