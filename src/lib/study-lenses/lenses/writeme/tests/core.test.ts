...
describe('writeme core', () => {
	...
	describe('applicability — always applicable', () => {
		it('returns true for a parseable snippet', () => {
			expect(writemeCore.applicability(embody('const x = 1;').facts)).toBe(
				true,
			);
		});

		it('returns true for an unparseable snippet', () => {
			expect(writemeCore.applicability(embody('const').facts)).toBe(true);
		});

		it('returns true for an un-tokenizable snippet', () => {
			expect(writemeCore.applicability(embody("'unterminated").facts)).toBe(
				true,
			);
		});
	});

	describe('recommend — deferred until the analysis surface lands', () => {
		it('returns an empty array', () => {
			expect(writemeCore.recommend(embody('let ok = 1;'))).toEqual([]);
		});

		it('returns a deep-frozen array', () => {
			expect(
				Object.isFrozen(writemeCore.recommend(embody('let ok = 1;'))),
			).toBe(true);
		});

		it('returns the shared reference across calls', () => {
			expect(writemeCore.recommend(embody('let ok = 1;'))).toBe(
				writemeCore.recommend(embody('const')),
			);
		});
	});
});
