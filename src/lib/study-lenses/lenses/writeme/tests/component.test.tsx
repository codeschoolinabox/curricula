 ...
 * so doc-level behavior (re-seed, the pristine-gate, learnerCode mirroring) IS
...
import writemeLens from '../index.jsx';
...
describe('writeme wrapper — mount', () => {
	describe('Zero — degenerate snippets do not crash', () => {
		...
		it('renders the lens root even on an unparseable snippet (no fallback panel)', () => {
			...
			expect(
				container.querySelector<HTMLElement>('[data-lens="writeme"]'),
			).not.toBeNull();
		});
	});
	...
	describe('Root reflects the committed config defaults', () => {
		it('reflects the default view mode (write)', () => {
			...
			expect(
				container.querySelector<HTMLElement>('[data-lens="writeme"]')?.dataset
					.viewMode,
			).toBe('write');
		});
		...
		it('reflects the default scaffold toggles (colorize/comments/diff on, suggestions off)', () => {
			...
			expect(root?.dataset.colorize).toBe('true');
			expect(root?.dataset.suggestions).toBe('false');
			expect(root?.dataset.comments).toBe('true');
			expect(root?.dataset.diff).toBe('true');
		});
		...
		it('reflects scaffold-toggle overrides on the root (triangulates each default)', () => {
			...
			expect(root?.dataset.colorize).toBe('false');
			expect(root?.dataset.suggestions).toBe('true');
			expect(root?.dataset.comments).toBe('false');
			expect(root?.dataset.diff).toBe('false');
		});
...
describe('writeme wrapper — view toggle + read view', () => {
...
describe('writeme wrapper — scaffold toggles (compartments)', () => {
	...
	it('keeps the read editor showing the solution across a diff toggle in read view', async () => {
	...
	// ─── diff toggle — write-editor overlay wiring ───
	...
	it('renders the Diff checkbox, checked by default', () => {
		...
		const diffToggle = container.querySelector<HTMLInputElement>(
...
describe('writeme wrapper — comments toggle + reset', () => {
	it('renders the comments checkbox (checked by default) and a Reset button', () => {
		...
		const comments = container.querySelector<HTMLInputElement>(
