// cspell:ignore keymap inited
/**
 * @file React wrapper for the `writeme` lens — default-exports the frozen
 * `Lens` object the composition root imports by reference.
 *
 ...
 * (deps `[]`); the learner's edits are mirrored into a `useRef` read at mount
 * time. The scaffold toggles (colorize / suggestions / diff) live-reconfigure
 * CodeMirror `Compartment`s and the keep-comments re-seed dispatches a doc
 * change — neither remounts, so no value can re-fire the mount effect and
 * destroy the view mid-typing (a per-keystroke remount is structurally
 * impossible here).
 */
...
const WritemeComponent: ComponentType<LensProperties> =
	function WritemeComponent({ embodiment, config }) {
		...
		// defaults. The solution is the source fact — a given stage that cannot
		// fail (`facts.source` carries no failure arm), read once here.
		const resolved = useMemo(() => writemeCore.config(config), [config]);
		const solution = embodiment.facts.source.value;
		...
		const solutionReference = useRef(solution);
		solutionReference.current = solution;
		...
		const startingTemplate = useMemo(
			() => (keepComments ? commentSkeleton(solution) : ''),
			[keepComments, solution],
		);
		...
		// once the learner types, the updateListener captures the doc here, and
		// the mount effect re-seeds from this (preserved) text via the ref.
				...
				if (host && compartment) {
					...
					const updateListener = EditorView.updateListener.of(
						function onUpdate(update) {
							...
							// orchestrator's snippet (read-only-view invariant).
		...
		useEffect(
			...
			[viewMode, colorize, diff, solution],
		);
		...
		function reseedWriteEditor(withComments: boolean) {
			const view = editorView.current;
			if (view) {
				const template = withComments ? commentSkeleton(solution) : '';
				view.dispatch({
