...
- The active tool persists across a view-toggle: switch to eraser in code-view,
  toggle to flowchart-view, eraser is still active. Both views accept all three
  tools. Rationale: the toggle is for changing what the learner is annotating,
  not what they're annotating with — kicking them back to pen on every view
  switch would interrupt an in-progress annotation gesture.
...
- **Initial view is parse-gated.** When `config.defaultView === 'flowchart'` but
  `embodiment.status.parsed === false`, the initial view degrades to `code`, so
  a learner never lands in a guaranteed-failing flowchart-view on mount.
  Together with the disabled toggle, an unparseable snippet is code-view-only
