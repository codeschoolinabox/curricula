...
taxonomy is the house taxonomy (ternary `?` / `:` are delimiters, not operators;
...
ever simultaneously multi-category is left open. Every category a token carries
is keyed to that token's `[start, end)` span.

**Home category / re-bin** — the _home category_ is what the classification
table assigns from the token type — currently the whole (single-entry)
...
`` ` ``, `${`/`}`), the index (into the returned array — valid only against the
full, unfiltered result) of the other half of the pair. Pairing is what
disambiguates a `}` that closes a block from a `}` that closes a template
expression (Acorn gives both the same `braceR` token), and it is the ground
truth for "click this brace's matching partner" exercises.
...
final row is a catch-all. Exotic constructs a naïve walk might silently skip
(the `*` in `import * as ns`, the `*` in `yield* g()`) classify as `operator` by
token type — a deliberate totality guarantee.

**Semantic categories vs. Acorn's lexer flags** — the taxonomy classifies by
what a token does in the NM, so it splits apart what Acorn's `.keyword` flag
conflates: `typeof` / `in` / `instanceof` / `void` / `delete` are `operator`;
`null` / `true` / `false` are `literal`; the namespace-import and delegation
stars are `operator`. The consequence is learner-visible: filtering blanks by
"keywords" does not hit `typeof`; by "operators" does — the semantically
accurate behavior.
...
- **Generator `*` re-bin**: a `*` token in function/method/property generator
  position is re-binned `delimiter` with role `generator` (house taxonomy). The
  `*` of `yield*` delegation and of `import * as ns` stays `operator` (role
  `other`).
...
const classified: readonly ClassifiedToken[] = classifyTokens({
	...
	tokens, // Acorn token stream
	ast, // Acorn Program node
});
```

`ClassifyInput` is declared in **acorn terms**
(`tokens: readonly acorn.Token[]`, `ast: acorn.Node`) — that is what the
classifier actually walks. A consumer holding an `Embodiment` reads these off
its facts (`facts.source.value` for `code`, `facts.tokens.value.tokens` for the
token stream, `facts.ast.value` for the AST); the projection is the consumer's
...
- **Pure.** No mutation of `tokens`, `ast`, or `code` — safe on deep-frozen
  embodiment data. The module writes no synthetic fields onto AST nodes.
...
- **Contextual keywords as plain names**: `of`, `get`, `set`, `from`, `as`,
  ...
  token-stream pass leaves it `keyword` for now (a deferred refinement pending
  AST context).
- **Open rulings (flagged, low priority — all outside JEJ except `new`)**: `new`
  ...
  pending a ruling.
- **Comments are not tokens** and do not appear in the output. (They travel on
  the parse's comment channel, not the token stream; classifying them is out of
  scope.)
...
`facts.environment`) too, while classifying takes three narrow values — it
underlies any consumer and must not assume one.

## Why this module exists

Token classification is a distinct concern from selection: the blanks lens needs
it to decide what is blankable, and the quiz lens's question generator needs the
same classification as quiz-anchor ground truth. Because two consumers need the
exact same exhaustive, pure classification — and only the classification — it
lives as a domain-blind `lib/` leaf that either can consume without an upward
dependency. Keeping it selection-free is what makes it shareable: a walk that
rolled probabilities, skipped config-excluded tokens, or mutated AST nodes en
route could not be.
...
- **Domain-blind leaf.** The module consumes the raw acorn parse shapes by
  value; it imports no package region — not embody, not levels, not lenses — not
  even for types. The `facts` projection to `{ code, tokens, ast }` is the
  consumer's boundary, never this module's concern.
