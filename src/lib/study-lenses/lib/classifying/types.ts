 ...
 * classifier actually walks. A consumer holding an `Embodiment`
 * projects these from its facts (`facts.source.value`,
 * `facts.tokens.value.tokens`, `facts.ast.value`); the narrowing is the
 * caller's one-line boundary. Tests construct the same shapes with a
 * direct `acorn.parse` call.
 */
