...
The engine's consumers (the evaluators) live in their own region. Reference
logic keeps the engine's suites self-contained: trivial worker logic that
injects `emit`/`call`/`getConfig`, a halt author that stamps `viaReference` and
recognizes one limit-throw shape (`ReferenceLimitError`), and thread logic that
drops one sentinel (`'reference:drop'`), echoes string calls, and refines
stamped limit halts. That is exactly enough surface to pin halt carriage,
refinement, drop-vs-yield, and the call channel — with no domain vocabulary
