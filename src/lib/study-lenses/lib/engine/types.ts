 ...
 * this contract — no type parameters. Consumers (the JEJ tracers, the embody
 * adapter) narrow at their own layer. The engine never reads a payload's
 ...
 *
 * Not, e.g., embody's EvaluateHandle — that consumer's adapter wraps this
...
/**
 * The engine's complete outcome vocabulary. Downstream values
 * ('limit-exceeded', 'not-runnable') are mappings applied by the layers
 * that own them, never settled here.
 */
...
type EngineError = {
	 ...
	 *   serializer, or a consumer setup failure.
