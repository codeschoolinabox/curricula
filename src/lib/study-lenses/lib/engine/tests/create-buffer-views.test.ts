...
describe('createBufferViews', () => {
	describe('control view', () => {
		...
		it('backs the control view with the buffer passed in', () => {
			const sab = new SharedArrayBuffer(PROTOCOL.BUFFER_SIZE);
			const views = createBufferViews(sab);

			expect(views.control.buffer).toBe(sab);
		});
	});
	...
	describe('payload view', () => {
		...
		it('backs the payload view with the buffer passed in', () => {
			const sab = new SharedArrayBuffer(PROTOCOL.BUFFER_SIZE);
			const views = createBufferViews(sab);

			expect(views.payload.buffer).toBe(sab);
		});
	});
});
