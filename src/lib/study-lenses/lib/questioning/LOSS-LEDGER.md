...
<!-- cspell:ignore reenrichment linearization PBSI PBIS deixis gradability -->
...
| § Bounded context                              | TRANSPORTED substance          | README § The two registers + § Ownership boundary | Purpose-row exclusivity, static decidability, AND the no-runtime-evaluation exclusion (initially dropped in transport; restored per design review — then OVERRULED as a kind law 2026-08-18: it survives only as today's engines' own mode; see the gate-redirect section below). Engine-API specifics stay for Stage 3. |
...
| §6 L0–L4 table, exit-point framing, intellectual-agency note                                         | TRANSPORTED | PEDAGOGY.md § The 5 layers                  | The table is load-bearing for the 5×5 SOLO reading, `Level.userExperience`, and L4-as-questioning — verbatim by default.                                   |
| §6's two framing sentences ("Each chapter runs ALL 5 layers." / "The layers are engagement depths…") | MERGED      | PEDAGOGY.md § The 5 layers                  | One sentence carrying both propositions, order swapped; the capitalized ALL flattens to "all five".                                                        |
...
## Source: this directory's landed docs + `lib/README.md` (questioning-DDD rewrite, 2026-08-18)

The DDD rewrite edits the landed parent docs; per the migration discipline's
corollary, its removals and rewords are enumerated here. Additions (the
`Questioner` envelope, `notional-machine.md`, `ux/user-journeys.md`, `tests/`,
the new application sections) are authored, not transported, and carry no loss.
Rulings of record are dated in DOCS.md § Decisions.

| Source content                                                                                     | Disposition | Destination                      | Justification                                                                                                                                                                                                                   |
| -------------------------------------------------------------------------------------------------- | ----------- | -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| README lead + throughout: the "two question engines" closed-roster framing                         | REWORDED    | README (family voice)            | Locked decision 6 (open family): the region is a kind, not a roster; instances named as instances. Swept: lead, § What lives here, § Navigation, glossary, `lib/README.md` entry + admissibility ¶, types.ts header.            |
| README § The BLOCK model: the two gloss tables                                                     | REMOVED     | — (value lists + pointer remain) | Human ruling 2026-08-18: the per-value glosses were byte-identical in three places; PEDAGOGY.md § The BLOCK model keeps the theory copy and types.ts JSDoc keeps the working copy. Headings survive; inbound citations resolve. |
| README § Taxonomies: "keyed to a finite JEJ concept … bounded because JEJ's concept set is finite" | REWORDED    | README § Taxonomies              | Locked decision 9: the bank is UN-bounded (grows toward all JS); `serves` replaces boundedness as the serve-this-code test. A deliberate semantic reversal executing the ruling.                                                |
| README glossary **engine** entry ("one of the two question libraries")                             | REWORDED    | README § Glossary                | Pre-merge review flag (closed-roster phrasing); SPEC § Terms questioner-vs-engine reading. The SPEC sentence claiming the old entries "survive under this reading" is superseded in detail — recorded in DOCS § Decisions.      |
| README glossary 3D-space entry: DOCS.md named as the space's home + ruling date                    | REWORDED    | README § Glossary                | The home is PEDAGOGY.md (decision 8, executed); the date parenthetical was transport-status voice — dates consolidate in DOCS § Decisions.                                                                                      |
| README § Ownership boundary: "Engines never import each other"                                     | REWORDED    | README § Ownership boundary      | Decision 7 consequence: a higher-order questioner imports the questioners it consumes, so the law narrows to leaves — composition never lives in a leaf.                                                                        |
| README § Conventions + DOCS § Structural constraints: "zero imports" invariant                     | REWORDED    | both files                       | Human ruling 2026-08-18 (Branch B): one type-only structural import; the surviving invariant is zero RUNTIME exports. Measured ground in DOCS § Decisions.                                                                      |
| DOCS § Shared engine shape (title + engine voice)                                                  | REWORDED    | DOCS § Shared questioner shape   | The envelope, serve/ask posture, and read-bound joined the sketch; constraints unchanged in substance.                                                                                                                          |
| DOCS "The region ships no tests of its own" + "four type aliases" rationale                        | REMOVED     | DOCS § Structural constraints    | Falsified by the envelope: `tests/` now carries the kind's type-contract assertions; the grid aliases' no-runtime-surface claim survives inside the new bullet.                                                                 |
| Ledger §§ "deprecated-architecture root DOCS § 3D" + "this directory's DOCS.md § 3D"               | RETIRED     | —                                | Burn-down: both sections' points are resolved in PEDAGOGY.md § The 3D Block Model space (decision 8); the relocation's own deviations were reviewed at the transport's AR-5.                                                    |
| Ledger § Owed forward rows 1–2 (README 3D-home pointers; duplicated gloss tables)                  | RETIRED     | —                                | Discharged by this rewrite (pointers fixed; tables trimmed under the 2026-08-18 ruling).                                                                                                                                        |
| `lib/README.md`: "holds the two question engines … shared pedagogy documentation"                  | REWORDED    | lib/README.md                    | Family framing + decision 8: the pedagogy home is PEDAGOGY.md; the parent carries the kind and questioning's application. Admissibility ¶ re-argued (family admission rule; "lifecycle-blind" restated as the read-bound).      |

## Gate redirect (2026-08-18): the permissive kind

At the Phase-0 human gate the maintainer overruled the
never-runs/static-only/deterministic laws the rewrite had codified (ruling
recorded in DOCS.md § Decisions). Rewords executing that ruling:

| Source content                                                                               | Disposition | Destination                              | Justification                                                                                                                                                        |
| -------------------------------------------------------------------------------------------- | ----------- | ---------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| notional-machine.md "reading machine, never a running machine" + READ box + static laws      | REWORDED    | notional-machine.md (asking machine)     | Questioners may run the code; GROUND replaces READ; determinism demoted to a property of the landed engine; the only-facts-or-config prediction clause dropped.      |
| README § Ownership boundary "Pure, synchronous, and deterministic" bullet                    | SPLIT       | README § Ownership boundary              | Frozen returns survive (repo deep-freeze law); purity/synchrony/determinism were means-legislation. The static-ground-truth bullet replaced by the no-learner form.  |
| README/DOCS closed charter "static decidability"                                             | REWORDED    | both files + glossary                    | Charter is machine-gradability; static decidability survives as today's closed engine's mode word (Stage-3 quarry language intact).                                  |
| README glossary **engine** "Pure, synchronous machinery"                                     | REWORDED    | README § Glossary                        | A dynamic questioner's engine may run the program; the entry now says reading, running, or both.                                                                     |
| README § Assessment is data opener "pure and stateless"                                      | REWORDED    | README § Assessment is data              | Decision 9's content is learner-statelessness + assessment-as-data; blanket generation-purity was over-reach. SPEC headline superseded-in-detail (DOCS § Decisions). |
| DOCS § Shared questioner shape "pure, synchronous transformation" + "Ground truth is static" | REWORDED    | DOCS sketch                              | Separate rows' worth in one sweep: the shape no longer legislates means; the data-flow edge labels drop ", pure" for the same reason.                                |
...
| README § Leveling "This parent and both engines are language-level-blind" | REWORDED    | README § Leveling                      | Pre-merge review catch: the sentence states a law of the KIND in roster voice; a future third questioner is equally bound — now "every questioner under it".                                                                                                                                                                                                                                   |
...
| Debt                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Owed to                               |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------- |
| The human's socratizing-twins intent, recorded at the Stage-3 gate (2026-08-18): "socratizing will be the same when we get there" — socratizing owes MODULE-level twin documents (its own notional-machine.md + ux/) at a future stage, matching quizzing's landed twin-doc: both standing value.                                                                                                                                                    | A future socratizing-twins session.   |
