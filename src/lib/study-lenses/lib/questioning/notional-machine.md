...
- **ASK, and its refusal arm.** The ask entry either runs the machine or refuses
  ...
  silent empty success. It may answer directly or behind a promise that settles
  as data (never a rejection) — await uniformly. Predict: unparseable source
  refuses at ask with the parser's cause; it does not produce zero items.
