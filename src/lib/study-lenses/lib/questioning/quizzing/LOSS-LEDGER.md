...
| `resolving/resolve-binding.ts` | ports verbatim + three mechanical conforms (landed U4; "two" corrected in place per this file's rule) | the type-import re-point to `./types.js` (with two prettier/import-order knock-ons); the type-name substitution to the minted names (ScopeAnalysis→ScopeForest ×2, ScopeInfo→ForestScope ×6, DeclarationInfo→TrackedDeclaration ×6, + one signature rewrap); a prepended cspell in-file ignore (`reassignability`, quarry-verbatim JSDoc word — the U2 precedent class). The walk (deepest-scope descent → climb → project) is byte-faithful |
