// ported from src/lib/study-lenses--deprecated-architecture/lib/quizzing/tests/descend-identifiers.test.ts
// @ blob cf2ae32c8ce7a2d1ce96f790dc19c08eae5219b4
// rewires: embody-facts fixtures, ast-from-facts
...
function astOf(facts: Facts): Node {
	if (!facts.ast.ok) {
		throw new Error('astOf requires parsed facts');
	}
	return facts.ast.value;
}

function anchorsOf(code: string): readonly IdentifierAnchor[] {
	return descendIdentifiers(astOf(embody(code).facts)).identifierAnchors;
}

function propertyAnchorsOf(code: string): readonly PropertyAccessAnchor[] {
	return descendIdentifiers(astOf(embody(code).facts)).propertyAccessAnchors;
}
...
describe('descendIdentifiers', () => {
	...
	describe('Simple', () => {
		it('is deterministic across repeated descents of the same AST', () => {
			const ast = astOf(embody('o.x; let y = 1; y;').facts);
