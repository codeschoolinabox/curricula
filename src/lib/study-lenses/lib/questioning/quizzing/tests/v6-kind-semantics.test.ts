// ported from src/lib/study-lenses--deprecated-architecture/lib/quizzing/tests/v6-kind-semantics.test.ts
// @ blob 59eb23688109cb8da9208d1e9ebdf74561ffb5a9
// rewires: embody-facts fixtures, classifying depth, classify-from-facts, why-comment vocabulary
// cspell:ignore misimplementation reassignable
...
describe('v6KindSemantics', () => {
	...
	describe('Boundaries', () => {
		...
		it('emits nothing for a var binding (non-JeJ, but parseable)', () => {
			// `var` is outside JeJ, but quizzing gates on parsed facts, not
			// validated ones, so a `var` snippet reaches the generators; the forest
		...
		it('skips only the var binding in a mixed let/var facts (guard is per-binding)', () => {
