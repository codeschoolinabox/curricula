 ...
 * `../../classifying`, `BlockCell` from the questioning parent
 * (`../types.js`), `NodePath` / `Facts` from `embody`.
 */

import type { Facts, NodePath } from '../../../embody/types.js';
import type { Category, ClassifiedToken } from '../../classifying/types.js';
import type { BlockCell } from '../types.js';
 ...
 * `(facts, classified, filter)`. `unlocks` lists the distinct `groupKey`
...
/**
 * Configuration for `generateQuiz`. Declared, not yet consumed —
 * `generateQuiz` accepts and ignores it (the no-op is oracle-pinned);
 * building the filter is a recorded future design event. Mirrors
 * socratizing's
 ...
 * - `range` is a zero-indexed half-open offset span `[start, end)` — an
 *   item is kept when its `anchorRange` overlaps it (any overlap, not
 *   containment; human ruling 2026-08-18, Stage-3 AR-1: flipped from the
 *   prior architecture's 1-based inclusive lines to the family's
 *   offset-native anchor law, matching the socratizing precedent);
 * - `count` caps the emitted-order result last (`0` ≡ omitted — no cap).
 */
...
/**
 * The content entry point. Runs the registered generators over the parsed
 * `facts` and their pre-computed `classified` tokens and returns a frozen
 * array in registry order, then stream order — deliberately not source
 * order. `filter` is accepted but not consumed (declared, not built — see
 * `QuizFilter`). Quizzing never calls `classifyTokens` — `classified`
 * arrives as a parameter (see README § Public API for the input asymmetry).
 * Throws on null, unparsed, or environment-defected input: it is called
 * behind its caller's gate (the token, tree, and environment stages all
 * ok), and a valid `classified` already implies a successful parse, so a
 * failed stage here is a caller bug to surface (the same posture as the
 * sibling `classifyTokens`).
 */
...
/**
 * The quizzing questioner's config — forward-declared as an object whose
 * fields are this questioner's own implementation choice (human ruling
 * 2026-08-18: the family sees config-is-an-object; each questioner owns its
 * fields as implementation). Declarative and serializable by the family's
 * config law, with no learner-model parameter. NO field is consumed yet —
 * the engine's `filter` is itself a declared no-op (oracle-pinned), so
 * passing any config today changes nothing; typed fields land when
 * filtering is built.
 */
export type QuizzingConfig = Readonly<Record<string, unknown>>;
