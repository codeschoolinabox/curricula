...
import type { ClassifiedToken, ClassifyInput } from '../../classifying/types.js';
...
describe('v7UsageKind', () => {
	...
	describe('Interfaces', () => {
		it('emits only mcq items', () => {
			...
			const items = runGenerators(
				buildContext(snippet, classifyOf(snippet)),
				[v7UsageKind],
			);
		...
		it('keys the propagation group per occurrence', () => {
			expect(v7ItemsOf('let x = 1;')[0]?.groupKey).toBe('usage:4-5');
		});

		it('keys two occurrences of one binding into distinct groups', () => {
			expect(
				v7ItemsOf('let x = 1; x;').map((item) => item.groupKey),
			).toEqual(['usage:4-5', 'usage:11-12']);
		});
