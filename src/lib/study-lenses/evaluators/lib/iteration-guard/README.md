...
Pure and synchronous throughout ⇒ **node tests only**. Call text is asserted at
the factory level (statement forms, loc-string encoding, single-line invariant),
with one thin integration case proving the delegation to loop-guard end-to-end —
loop-guard's own three gates already pin placement, so placement is not
re-covered here. The guard state is driven directly as functions: counter
