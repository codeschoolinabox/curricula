 ...
 * GUARD-SPLICED source (iteration guards splice FIRST, on the original
 * text; the splice order is not interchangeable — the ledger's retained
 * pins intercept:356/:361).
 *
 * Rewrites every call expression it can safely enclose into a call to the
 * `__$lc` helper the worker setup implements (the deprecated port's Seam 5
 * protocol; the greenfield worker setup lands beside this file), carrying
 * the call's own span AND offset pair READ FROM THE ORIGINAL learner text —
 * placement is computed against the guarded text, whose positions are
 * shifted on every line a guard or reset call was spliced into, while the
 * coordinates reported are the learner's own, in the facts' coordinate
 * space (README § Glossary — the wrap). The two readings are reconciled
 * (the deprecated port's briefing decision B-2): one syntactic eligibility
 * predicate runs over both parses' call lists, and a disagreement about
 * which wrap-eligible calls exist throws the typed boundary error below —
 * a machinery defect the settlement maps as
 * `'defect'`/`'unreachable-outcome'`, never a silently shifted span.
 *
 * The wrap preserves the program's meaning or declines (instrumentation
 * assumed sound, HR-19 — the wrap never changes what runs; the decline
 * roster is the deprecated port's briefing decision B-3): a call whose
 * `await`/`yield` belongs to the enclosing function, one interior to an
 * optional chain's spine, a `super(…)` call, and direct `eval` are left as
 * the learner wrote them and carry no stamp. A `__$`-prefixed callee name
 * marks the guard protocol's own calls, which are skipped, never wrapped.
 * The rewrite is bottom-up, so nesting needs no offset bookkeeping, and
 * every insertion is same-line — lines are preserved 1:1.
 *
 * The stamp rides ENCODED as ONE string — `'L:C:L:C:S:E'`: the region's
 * span encoding (1-based lines / 0-based columns) with the UTF-16 offset
 * pair appended (the HR-12 addition — stamped near-free from the original
 * parse, making the `byOffset` join direct; acorn's `start`/`end` are
 * native UTF-16 code-unit offsets). One stamp carries span and offsets
 * together, so no moment can ever hold one without the other (README § The
 * events — `loc`, `start`/`end`, and `nodePath` are `null` TOGETHER
 * wherever no wrap attributed); it is decoded only where a record or a
 * halt needs it, worker-side: the first four fields are the span, the last
 * two the offset pair.
 */
 ...
 *   spans and offsets are read from; `sourceType` is the snippet's own
 *   parse goal — fixed, never inferred, so a program that parsed upstream
...
export default function wrapCallExpressions(sources: WrapSources): string {
	...
	const encodedStamps = new Map<Node, string>();
	for (const [index, call] of eligible.entries()) {
		encodedStamps.set(call, encodeStamp(originalEligible[index]));
	}
...
type RewriteContext = {
	...
	/** Wrapped call → its encoded stamp. Membership IS the wrap set — one
	 * structure, so a wrapped call can never lack its pairing. */
	readonly encodedStamps: ReadonlyMap<Node, string>;
};
...
/** Fixed parse goal, loud on failure — never a fallback (the same goal the
 * snippet was parsed with, so a program that parsed upstream parses here). */
 ...
 * both readings share (the deprecated port's briefing decision B-2). A
 * call is eligible unless: its callee name carries
 * the `__$` prefix (the guard protocol's own calls); its callee is `Super`
 * or direct `eval` (a call whose scope is its own call site); it sits
 * anywhere inside an optional chain's spine (wrapping an interior link
 * would defeat the chain's short-circuit); or an `await`/`yield` belonging
 * to the enclosing function sits anywhere in its own subtree (the wrap's
 * arrow would change what suspends, or not parse).
 */
 ...
 * wrap would defeat the short-circuit (the deprecated port's ruling H-5). A
 * call reached off any other key, an argument especially, leaves the spine
 ...
 * enclosed as `__$lc('<stamp>', () => <interior>)`, its own interior
...
function rewriteInterior(
	...
	const topmost = collectTopmostWrapped(container, context.encodedStamps);
	...
	for (const call of topmost) {
		...
		result += `__$lc('${stampOf(call, context)}', () => ${rewriteInterior(call, call.start, call.end, context)})`;
		cursor = call.end;
	}
	return result + context.source.slice(cursor, end);
}

/** The pairing read, loud on the impossible miss — a silently empty stamp
 * would be the exact mis-attribution this module exists to prevent. */
function stampOf(call: Node, context: RewriteContext): string {
	const stamp = context.encodedStamps.get(call);
	if (stamp === undefined) {
		throw new Error(
			'wrapCallExpressions: a wrapped call lost its stamp pairing — a machinery defect, never a learner condition',
		);
	}
	return stamp;
}
...
function collectTopmostWrapped(
	container: Node,
	encodedStamps: ReadonlyMap<Node, string>,
): Node[] {
	const topmost: Node[] = [];
	eachChildNode(container, function findTopmost(child) {
		if (encodedStamps.has(child)) {
			topmost.push(child);
			return;
		}
		topmost.push(...collectTopmostWrapped(child, encodedStamps));
	});
	return topmost;
}
...
/** `'L:C:L:C:S:E'` — the region's span encoding with the offset pair
 * appended, every field read from the ORIGINAL text's parse: 1-based
 * lines, 0-based columns, UTF-16 code-unit offsets (acorn's own
 * `start`/`end`). */
function encodeStamp(call: Node): string {
	...
	return `${loc.start.line}:${loc.start.column}:${loc.end.line}:${loc.end.column}:${call.start}:${call.end}`;
}
 ...
 * settlement maps as 'defect'/'unreachable-outcome', never a silently
 * shifted span. */
