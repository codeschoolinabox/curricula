 ...
 * worker logic. This is the per-consumer entry the engine's worker-factory
 * pattern loads (engine README § Public API): the thread side constructs
 ...
 * out of run's THREAD-side modules, where the seam boundary lives; a worker
 * entry is by definition the file that wires the bootstrap (the deprecated
 * entry's recorded ruling, human ruling 2026-07-30, R-5 — carried).
 */
