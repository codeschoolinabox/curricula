...
/**
 * @file R5's Node tier: the refusal, proven in the environment that is
 * actually refused.
 *
 * Node has SharedArrayBuffer but no Worker
 * (`typeof Worker === 'undefined'`, `typeof SharedArrayBuffer === 'function'`),
 * so this tier reaches the worker arm of the probe and CANNOT reach the
 * shared-memory arm — the test names say which is which. Everything main
 * wires past the probe is evidenced in `index.browser.test.ts` over the real
 * transport.
 *
 * Triangulation, stated honestly: every row in THIS file passes against a
 * hardcoded always-refuse `main`, because a constant reason string containing
 * "Worker" satisfies the wording row without anything ever reading `typeof
 * Worker`. The row that kills that fake lives in the other file —
 * `does not refuse where Worker and shared memory both exist` — which is the
 * environment-boundary crossing DEV.md sanctions (move the test file, never
 * mock the environment away). The two files triangulate as a pair, and neither
 * does it alone.
 */
...
describe('run evaluator (node — no Worker)', () => {
	describe('identity', () => {
		it('is named run', () => {
	...
	describe('applicability', () => {
		it.each([
			...
			(execution, iterations) => {
				...
				expect(
					run.applicability(specFor('1 + 1;', execution, iterations)),
				).toBe(true);
			},
		);
	});
	...
	describe('the environment refusal', () => {
		...
		it('names the missing Worker capability in its reason', () => {
			// PINNED(D8-as-widened, human-ratified 2026-07-28: applicability stays constant-true; the environment answer is ONE refusal at main naming the missing capability)
			expect(refusalOf(specFor('1 + 1;')).reason).toContain('Worker');
		});

		it('refuses without throwing', () => {
			expect(() => run.main(specFor('1 + 1;'))).not.toThrow();
		});
