...
describe('drift guard against upstream JEJ surface', () => {
	it('KEYWORD_LABELS matches KEYWORDS from collect-jej-surface', () => {
		expect([...KEYWORD_LABELS].toSorted((a, b) => a.localeCompare(b))).toEqual(
			[...KEYWORDS].toSorted((a, b) => a.localeCompare(b)),
		);
	});
	...
	it('GLOBAL_LABELS matches allowedGlobals minus SUPPRESSED_GLOBALS and minus KEYWORDS', () => {
		...
		expect([...GLOBAL_LABELS].toSorted((a, b) => a.localeCompare(b))).toEqual(
			expected.toSorted((a, b) => a.localeCompare(b)),
		);
	});

	it('MEMBER_LABELS matches CURATED_MEMBERS from collect-jej-surface', () => {
		expect([...MEMBER_LABELS].toSorted((a, b) => a.localeCompare(b))).toEqual(
			[...CURATED_MEMBERS].toSorted((a, b) => a.localeCompare(b)),
		);
	});
