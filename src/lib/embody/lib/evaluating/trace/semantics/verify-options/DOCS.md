...
Keeps validation logic and tests co-located. If new constraints are added, each
can get its own helper file.
...
producing a silent empty result. Catching this early gives the consumer a clear
error message.
...
for start and `{ line: n, column: Number.MAX_SAFE_INTEGER }` for end. Then
compare `(start.line, start.column) ≤ (end.line, end.column)`.
...
- Range absent → passes
- Config is non-object or null → passes (defensive; schema validation runs
  first)
...
`options`) — NOT inside `options`. `verifyOptions` receives the full
`TraceConfig`, not just the inner `options` object.
