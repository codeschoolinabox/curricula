...
Two layers build on it today (neither lives in this module; the engine knows
neither):

- **JEJ tracers** — the evaluate tiers (run / intercept / trace.\*): they supply
  instrumented code, worker logic (traps, advice, mocks), and thread logic
  (event interpretation, whitelisting, limit refinement).
- **The embody adapter** — `embody/index.ts` composition: Snippet projection,
  the not-runnable short-circuit (a gated snippet never invokes the engine),
  `RunInstance` assembly, and the embody `EndReport` vocabulary.

Light cases use the engine directly: code, a worker entry, a few thread hooks —
no tiers, no contract, no ceremony.
...
The engine's `EngineHandle` is not embody's `EvaluateHandle` — the adapter
consumes the former to implement the latter. Same idea, different layers,
...
consumer setup failure), `call-error` (a round-trip that could not be serviced),
or `hook-error` (a throwing thread hook) — never by prose that a consumer would
have to string-match. One corner carries both: a `refineError` throw keeps the
existing halt alongside a hook-error.
...
vocabularies (embody's `limit-exceeded`, `not-runnable`) are mappings applied by
...
requirements on them; recording those here keeps tier and adapter authors
honest:
...
- The **refinement** is structured data, rich enough to distinguish an
  ...
  matching — the misclassification gate survives at the tracer and adapter
  layers, not in the engine.
- The **halt payload** carries the run metrics only the worker knows — iteration
  ...
  halt; a downstream owner defaults such metrics (e.g. the embody adapter's
  iterationCount 0).
- The halt payload is what downstream errors are built from (e.g. embody's
  `EmbodyError`, built by its adapter).
- Outcome rewrites belong downstream (e.g. the embody adapter's **errored +
  refinement → limit-exceeded**); the engine settles `errored` either way.
- `failReason` is stored by reference and is NOT deep-frozen by the engine; a
  downstream owner's deep pass (e.g. the embody adapter's `RunInstance`
  deep-freeze) is the authoritative freeze for its own data (consumers pass a
  clone if the original must stay mutable).

## Pause economics

An honesty rule for tier authors: **every emit costs a full pause round-trip
...
thousands of moments) MUST gate worker-side, before emitting. That is tier
...
- **termination cause** — the thread-side stop: cancel, fail (with payload),
  ...
  consumer setup failure), call-error (round-trip unserviceable), hook-error
  (throwing thread hook). First-write-wins with the halt — exactly one stop
  settles a run; anything later (including after settlement) is a no-op.
...
the adapter wraps the former to implement the latter. A worker _crash_ is not a
_halt_ — crash is the worker-error termination cause; halts are
