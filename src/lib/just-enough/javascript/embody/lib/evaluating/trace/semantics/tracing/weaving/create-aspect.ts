...
function createAspect(
	...
	const wrap =
		tagMap.size > 0
			? function withResolution(fn: PointcutEntry['pointcut']) {
					return wrapPointcut(fn, tagMap);
				}
			: function passthrough(fn: PointcutEntry['pointcut']) {
					return fn;
				};

	// --- Always: scope tracking hooks ---

	pointcut['_jej_block_setup'] = {
		kind: 'block@setup',
		pointcut: wrap(blockPointcut),
	};
	adviceGlobals['_jej_block_setup'] = blockSetup;

	pointcut['_jej_block_declaration'] = {
		kind: 'block@declaration',
		pointcut: wrap(blockPointcut),
	};
	adviceGlobals['_jej_block_declaration'] = blockDeclaration;

	pointcut['_jej_block_teardown'] = {
		kind: 'block@teardown',
		pointcut: wrap(blockPointcut),
	};
	...
	if (isAnyScopeDispatchEnabled(config)) {
		pointcut['_jej_block_after'] = {
			kind: 'block@after',
			pointcut: wrap(blockPointcut),
		};
		adviceGlobals['_jej_block_after'] = blockAfter;

		pointcut['_jej_block_throwing'] = {
			kind: 'block@throwing',
			pointcut: wrap(blockPointcut),
		};
	...
	if (isAnyExpressionEnabled(config)) {
		...
		pointcut['_jej_expression_after'] = {
			kind: 'expression@after',
			pointcut: wrap(expressionPointcut),
		};
	...
	if (isAnyApplyEnabled(config)) {
		pointcut['_jej_apply_around'] = {
			kind: 'apply@around',
			pointcut: wrap(applyPointcut),
		};
	...
	if (isAnyEffectEnabled(config)) {
		...
		pointcut['_jej_effect_before'] = {
			kind: 'effect@before',
			pointcut: wrap(effectPointcut),
		};
		adviceGlobals['_jej_effect_before'] = effectBefore;

		pointcut['_jej_effect_after'] = {
			kind: 'effect@after',
			pointcut: wrap(effectPointcut),
		};
	...
	if (isAnyStatementEnabled(config)) {
		...
		pointcut['_jej_statement_before'] = {
			kind: 'statement@before',
			pointcut: wrap(stmtPointcut),
		};
