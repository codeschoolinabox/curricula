...
import {
	best_lise,
	best_lise_inverse_indices,
	find_lises,
	patience_sort,
} from '../lib/lis.js';

/**
 * Invariant of `best_lise_inverse_indices`: removing the returned indices from
	...
	describe('best_lise_inverse_indices', () => {
		...
		describe('Many — already increasing leaves nothing to move', () => {
			...
			it('flags 3 of 4 for a fully-reversed sequence [3,2,1,0]', () => {
				...
				// regression anchor.
				expect(best_lise_inverse_indices([3, 2, 1, 0])).toEqual([1, 2, 3]);
			});
			...
			it('flags the two displaced lines in [2,0,3,1,4]', () => {
				...
				expect(best_lise_inverse_indices([2, 0, 3, 1, 4])).toEqual([1, 3]);
			});
		});

		describe('Boundaries / Interfaces — invariants on the result', () => {
			it('returns ascending indices', () => {
				const inverse = best_lise_inverse_indices([2, 0, 3, 1, 4]);
			...
			it('leaves a strictly-increasing subsequence after removing the inverse', () => {
				for (const input of [
					...
					expect(keptIsStrictlyIncreasing(input, best_lise_inverse_indices(input))).toBe(
						true,
					);
				}
			});
			...
			it('kept count equals input length minus inverse length', () => {
				...
				const inverse = best_lise_inverse_indices(input);
				const drop = new Set(inverse);
			...
			it('handles duplicate values (equal model positions) without error', () => {
				const inverse = best_lise_inverse_indices([0, 1, 1, 2]);
				expect(keptIsStrictlyIncreasing([0, 1, 1, 2], inverse)).toBe(true);
			});
		});
	});

	describe('best_lise — selection restores the original _.max tie-break', () => {
		it('prefers the highest-consecutive-run LIS regardless of candidate position', () => {
			...
			expect(best_lise([[1, 2], [0, 2]])).toEqual([1, 2]); // higher-score first
			expect(best_lise([[0, 2], [1, 2]])).toEqual([1, 2]); // higher-score last
		});

		it('prefers the longer-scoring run [0,1,2] over a singleton [5]', () => {
			expect(best_lise([[5], [0, 1, 2]])).toEqual([0, 1, 2]);
		});

		it('returns first-max on a true tie (both score 0)', () => {
			expect(best_lise([[3], [7]])).toEqual([3]);
		});
	});

	describe('find_lises — enumerates the full max-length candidate set', () => {
		it('returns BOTH equal-length LISes for decks [[1,0],[2]]', () => {
			// patience_sort([1,0,2]) = [[1,0],[2]]; max LIS length 2;
			// candidates are [1,2] and [0,2] — both must be returned so
			// best_lise can choose between them.
			const result = find_lises([[1, 0], [2]]);
