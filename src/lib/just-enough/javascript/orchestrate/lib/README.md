...
Each subdir keeps its own `README.md` + `DOCS.md`; cross-link from
this index.
...
Every public function in this directory takes `embodiment: Snippet`
as its first parameter (or the only parameter, depending on the
function). This is the central architectural decision documented in
