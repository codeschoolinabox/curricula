...
`'rejection'` severity is mapped to CM's `'warning'` — JEJ-subset
violations are teaching-boundary signals (yellow/orange gutter
marker), not syntax errors. Parse failures (`'error'`) stay red.
