...
Violations have a `severity` of `'error'` or `'warning'`.

- **Errors** indicate disallowed syntax — features outside the language level. A
  program with any errors is **invalid** (`isValid: false`).
...
- **Undeclared globals** (error): identifiers referenced but not declared and
...
These AST node types are NOT in the allowed list and always produce errors. Some
