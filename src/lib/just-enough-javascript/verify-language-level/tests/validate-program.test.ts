...
describe('validateProgram', () => {
	...
	describe('severity affects isValid', () => {
		it('is valid when all violations are warnings', () => {
			// custom level where Literal produces a warning instead of an error
		...
		it('is invalid when there are error-severity violations', () => {
			const report = validateProgram('var x = 5;', justEnoughJs);
			const errors = report.violations.filter((v) => v.severity === 'error');
		...
		it('var declaration produces error-severity violations', () => {
			const report = validateProgram('var x = 5;\nconsole.log(x);', justEnoughJs);
			const errors = report.violations.filter((v) => v.severity === 'error');
			expect(errors.length).toBeGreaterThan(0);
			expect(errors[0].nodeType).toBe('VariableDeclaration');
		});
	});
