 ...
 * 1. Body must be `BlockStatement` — **error** (same as if/while).
 ...
 *
 * If both apply (braceless + let), the error takes priority — the
...
/**
 * Validates a member expression: computed access (bracket indexing)
 * is always allowed, non-computed dot access must use an allowed
 * property name.
 *
 * @remarks Closes over {@link ALLOWED_MEMBER_NAMES} from the
 * enclosing module. No type checking — `.log` on a string is a
 * runtime error, not our problem. We only check the property name.
 */
function validateMemberExpression(node: Node): true | Violation {
	const record = node as unknown as Record<string, unknown>;
	...
	if (ALLOWED_MEMBER_NAMES.has(property.name)) return true;
 ...
 * {@link parseProgram} guarantees). Duplicated from
 * `collect-violations.ts` to keep this file self-contained — the
 * validators here don't import from `collect-violations`.
 */
 ...
 * referenced by {@link validateMemberExpression}.
 *
 * @remarks Extracted as a module-level Set so the validator can
 * close over it. Matches `allowedMemberNames` in the level config
 * below — both exist because the validator needs fast lookup while
 * the config exposes the Set for external consumers (like scope
 * analysis).
 */
...
const justEnoughJs: LanguageLevel = Object.freeze({
	...
	allowedMemberNames: Object.freeze(ALLOWED_MEMBER_NAMES),
