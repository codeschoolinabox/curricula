...
describe('validate', () => {
	...
	describe('scriptMode is dropped by the shaper', () => {
		it('does not surface scriptMode on with-statement programs', () => {
			// validate-program internally accepts script-mode for `with`,
			// but BaseResult deliberately drops the scriptMode marker.
			const result = validate('with (Math) { let x = PI; }\n');
