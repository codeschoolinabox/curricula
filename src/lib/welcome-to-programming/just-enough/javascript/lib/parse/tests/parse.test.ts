...
describe('parse', () => {
	...
	describe('with statement fallback', () => {
		...
		it('sets scriptMode to true for with statement code', () => {
			const result = parse('with (Math) { let x = PI; }\n');
			if (!result.ok) throw new Error('expected ok');
		...
		it('reports module error when both modes fail', () => {
			// Syntactically broken in BOTH module and script mode —
			// covers the script-mode-also-fails branch.
			const result = parse('with (Math) { let = ; }\n');
			expect(result.ok).toBe(false);
		});

		it('does not set scriptMode on plain module-mode success', () => {
			const result = parse('let x = 5;\n');
			if (!result.ok) throw new Error('expected ok');
			expect('scriptMode' in result).toBe(false);
		});
	});
