...
See `../config.types.ts` for the full `TraceOptions` type definition.
See `../options.schema.json` for the JSON Schema used by this pipeline.

## Key behaviors

- **Recursive shorthand expansion**: `{ expression: false }` expands into the full
  nested structure with all boolean leaves set to `false`, recursing into all
  sub-objects. `{ operators: { arithmetic: false } }` expands only the affected
  leaf; siblings keep their default `true`. Filter arrays and non-boolean/non-object
  properties are omitted — `fillDefaults` provides them via schema defaults.
...
- [../config.types.ts](../config.types.ts) — TraceOptions type with layer documentation
- [../options.schema.json](../options.schema.json) — JSON Schema (source for default-filling)
