...
Each profile isolates one semantic vertical (scopes, expressions, resolves, etc.)
to test a specific expected behavior in isolation.
...
`trace/tests/profiles/profiles.ts` exports a **single default object** containing all 11 profiles:
...
One default export (per AGENTS.md convention); all profiles in one frozen plain object.
...
Each file runs its profile against a fixed set of demo programs and asserts the complete event stream.
...
- **T6 (semantic-equivalence)** — imports the full profiles object; uses multiple entries to verify equivalent configs produce identical event streams (e.g. `{ expression: true }` vs `{ expression: { variables: true, literals: true, operators: true, ... } }`)
- **T5 (schema-conformance)** — uses `ALL_ON` to exercise every event type in a single trace; validates each event against its category schema
- **T4 test files** — each imports profiles and destructures the specific ones it needs
