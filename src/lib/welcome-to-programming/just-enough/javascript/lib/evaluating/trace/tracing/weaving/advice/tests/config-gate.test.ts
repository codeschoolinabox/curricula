...
describe('filter arrays', () => {
	it('isBindingGateOpen passes when filter is empty', () => {
		const config = { bindings: { kind: { let: true }, events: { read: true }, filter: [] } };
		expect(isBindingGateOpen(config, 'let', 'read', 'x')).toBe(true);
	});

	it('isBindingGateOpen passes when name is in filter', () => {
		const config = { bindings: { kind: { let: true }, events: { read: true }, filter: ['x', 'y'] } };
		expect(isBindingGateOpen(config, 'let', 'read', 'x')).toBe(true);
	});

	it('isBindingGateOpen fails when name is not in filter', () => {
		const config = { bindings: { kind: { let: true }, events: { read: true }, filter: ['x', 'y'] } };
		expect(isBindingGateOpen(config, 'let', 'read', 'z')).toBe(false);
	});

	it('isBindingGateOpen passes when no varName provided', () => {
		const config = { bindings: { kind: { let: true }, events: { read: true }, filter: ['x'] } };
		expect(isBindingGateOpen(config, 'let', 'read')).toBe(true);
	});

	it('isOperatorEnabled passes when filter is empty', () => {
		const config = { operators: { pure: { arithmetic: true }, filter: [] } };
		expect(isOperatorEnabled(config, 'pure.arithmetic', '+')).toBe(true);
	});

	it('isOperatorEnabled fails when operator not in filter', () => {
		const config = { operators: { pure: { arithmetic: true }, filter: ['+', '-'] } };
		expect(isOperatorEnabled(config, 'pure.arithmetic', '*')).toBe(false);
	});

	it('isPropertyAccessEnabled respects filter', () => {
	...
	it('isFunctionEnabled respects filter', () => {
