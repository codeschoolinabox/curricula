/**
 * @file SharedArrayBuffer encode/decode utilities for the run engine's
 * I/O protocol.
 *
 ...
 *
 * @remarks
 * Trapless variant — the SAB layout matches `intercept/worker-protocol.ts`
 * for symmetry, but only the I/O handshake slots are used. There is no
 * pause/event-ready protocol because the engine doesn't yield events.
 *
 * Buffer layout (8192 bytes total):
 *
 * | Index/Offset | View       | Purpose                                    |
 * | ------------ | ---------- | ------------------------------------------ |
 * | control[0]   | Int32Array | I/O signal: 0=idle, 1=waiting, 2=responded |
 ...
 * | control[4]   | Int32Array | (unused — reserved for layout symmetry)    |
 * | control[5]   | Int32Array | (unused — reserved for layout symmetry)    |
 ...
 *
 * @remarks Layout matches intercept's: 6 Int32 slots in control,
 * payload starts at byte 24. The trailing 2 control slots are unused
 * by run but kept for layout symmetry with the intercept engine.
 */
