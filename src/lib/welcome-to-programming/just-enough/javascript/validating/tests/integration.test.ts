...
describe('integration: common student mistakes', () => {
	...
	it('allows += operator (compound assignment)', () => {
		const report = validateProgram('let x = 0;\nx += 1;\n', justEnoughJs);
		expect(report.isValid).toBe(true);
	});
...
describe('integration: disallowed globals', () => {
	...
	it('allows eval as easter egg global', () => {
...
describe('integration: with statement easter egg', () => {
	it('accepts with statement program in script mode', () => {
		const report = validateProgram(
	...
	it('accepts arbitrary syntax inside with body without violations', () => {
		const report = validateProgram(
			'with ({}) { function foo() {} }\n',
			justEnoughJs,
		);
