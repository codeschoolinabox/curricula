...
describe('validateProgram', () => {
	...
	describe('dual-mode parsing (with statement)', () => {
		it('parses with statement in script mode and sets scriptMode', () => {
			const report = validateProgram(
				'with ({}) { let x = 1; }\n',
				justEnoughJs,
			);
			expect(report.parseError).toBeUndefined();
			expect(report.scriptMode).toBe(true);
		});

		it('normal programs parse as module without scriptMode', () => {
			const report = validateProgram('let x = 1;\n', justEnoughJs);
			expect(report.parseError).toBeUndefined();
			expect(report.scriptMode).toBeFalsy();
		});

		it('unrelated syntax errors still report parse error', () => {
			const report = validateProgram('let = ;', justEnoughJs);
			expect(report.isValid).toBe(false);
			expect(report.parseError).toBeDefined();
		});

		it('with program that also has unrelated syntax error reports parse error', () => {
			const report = validateProgram('with ({}) { let = ; }', justEnoughJs);
			expect(report.isValid).toBe(false);
			expect(report.parseError).toBeDefined();
		});

		it('nested with inside with parses and sets scriptMode', () => {
			const report = validateProgram(
				'with ({}) { with ({}) { let x = 1; } }\n',
				justEnoughJs,
			);
		...
		it('module syntax rejected even if script fallback succeeds without with', () => {
			// code that fails module parse for non-with reasons and has no with
			// statement in script parse — should keep the module error
			const report = validateProgram('let x = 010;', justEnoughJs);
			// octal literals are forbidden in strict/module mode
			// script mode allows them, but no WithStatement → keep module error
			expect(report.parseError).toBeDefined();
		});
	});
