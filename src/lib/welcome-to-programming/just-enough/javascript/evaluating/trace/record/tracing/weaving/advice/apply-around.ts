 ...
 */
function computeBinaryCoercion(operator: string, left: unknown, right: unknown): [unknown, unknown] | undefined {
 ...
 */
function computeUnaryCoercion(operator: string, operand: unknown): [unknown] | undefined {
...
function applyAround(
	...
	try {
		...
		if (discriminant === 'aran.performBinaryOperation') {
			...
			if (subkind && isOperatorEnabled(state.config, `pure.${subkind}`, operator)) {
			...
			if (isPropertyAccessEnabled(state.config, accessKind, String(key))) {
				const isShortCircuited = accessKind === 'optionalChaining' && (object === null || object === undefined);
