...
| apply-around | Conditional | Any of: `config.operators.pure.*`, `config.operators.shortCircuiting`, `config.operators.assignment`, `config.propertyAccess.*`, `config.functions.*`, `config.templates.*` | |
