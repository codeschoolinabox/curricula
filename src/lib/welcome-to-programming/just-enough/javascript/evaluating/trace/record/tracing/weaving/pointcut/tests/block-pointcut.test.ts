...
describe('blockPointcut', () => {
	...
	describe('point data shape', () => {
		...
		it('extracts user label from node.labels', () => {
			const result = blockPointcut(
				{ tag, labels: ['outer'] },
				{ type: 'WhileStatement' },
				null,
			);
			expect(result[4]).toBe('outer');
		});

		it('skips mangled labels (starting with $)', () => {
			const result = blockPointcut(
				{ tag, labels: ['$break', '$continue', 'myLabel'] },
				{ type: 'WhileStatement' },
				null,
			);
			expect(result[4]).toBe('myLabel');
		});

		it('returns null when no user labels', () => {
			const result = blockPointcut(
				{ tag, labels: ['$break'] },
				{ type: 'WhileStatement' },
				null,
