...
function parseEntry(
	...
	if (typeof prefix === 'string') {
		...
		// Exception: creation phase error (Acorn parse failure)
		if (prefix === '-> creation phase error:') {
			return {
				operation: 'exception',
				name: null,
				operator: null,
		...
		// Exception: execution phase error (escaped ADVICE traps)
		if (prefix === '-> execution phase error:') {
			return {
				operation: 'exception',
				name: null,
				operator: null,
		...
		// Exception: worker-level error (worker crash, module load failure)
		if (prefix === '-> worker error:' || prefix === '-> worker execution error:') {
			return {
				operation: 'exception',
				name: null,
				operator: null,
