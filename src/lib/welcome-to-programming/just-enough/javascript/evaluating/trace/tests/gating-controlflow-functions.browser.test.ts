...
describe('controlFlow gating', () => {
	describe('kind gating — conditionals', () => {
		it('disabling controlFlow.kind.conditionals removes conditional events', async () => {
			const config = withOverride(ALL_ENABLED, 'controlFlow.kind.conditionals', false);
			const { events } = await drainGenerator(
				'let x = 5;\nif (x > 3) {\n  let y = 1;\n}\nlet i = 0;\nwhile (i < 2) {\n  i = i + 1;\n}\n', config,
			);
			const conditionalEvents = events.filter(
				(e) => e.category === 'controlFlow' && (e as Record<string, unknown>).kind === 'conditional',
			);
			const whileEvents = events.filter(
				(e) => e.category === 'controlFlow' && (e as Record<string, unknown>).kind === 'while',
			);
	...
	describe('kind gating — loops', () => {
		it('disabling controlFlow.kind.loops.while removes while events', async () => {
			const config = withOverride(ALL_ENABLED, 'controlFlow.kind.loops.while', false);
			const { events } = await drainGenerator(
				'let i = 0;\nwhile (i < 2) {\n  i = i + 1;\n}\nlet x = 5;\nif (x > 3) {\n  let y = 1;\n}\n', config,
			);
			const whileEvents = events.filter(
		...
		it('disabling controlFlow.kind.loops.doWhile removes doWhile events', async () => {
			const config = withOverride(ALL_ENABLED, 'controlFlow.kind.loops.doWhile', false);
			const { events } = await drainGenerator(
				'let i = 0;\ndo {\n  i = i + 1;\n} while (i < 2);\nlet x = 5;\nif (x > 3) {\n  let y = 1;\n}\n', config,
			);
			const doWhileEvents = events.filter(
				(e) => e.category === 'controlFlow' && (e as Record<string, unknown>).kind === 'doWhile',
			);
			const conditionalEvents = events.filter(
				(e) => e.category === 'controlFlow' && (e as Record<string, unknown>).kind === 'conditional',
			);
		...
		it('disabling controlFlow.kind.loops.for removes for events', async () => {
			const config = withOverride(ALL_ENABLED, 'controlFlow.kind.loops.for', false);
			const { events } = await drainGenerator(
				'for (let i = 0; i < 2; i = i + 1) {\n  let x = i;\n}\nlet a = 5;\nif (a > 3) {\n  let b = 1;\n}\n', config,
			);
			const forEvents = events.filter(
				(e) => e.category === 'controlFlow' && (e as Record<string, unknown>).kind === 'for',
			);
			const conditionalEvents = events.filter(
				(e) => e.category === 'controlFlow' && (e as Record<string, unknown>).kind === 'conditional',
			);
		...
		it('disabling controlFlow.kind.loops.forOf removes forOf events', async () => {
			const config = withOverride(ALL_ENABLED, 'controlFlow.kind.loops.forOf', false);
			const { events } = await drainGenerator(
				'for (const c of "ab") {\n  let x = c;\n}\nlet a = 5;\nif (a > 3) {\n  let b = 1;\n}\n', config,
			);
			const forOfEvents = events.filter(
				(e) => e.category === 'controlFlow' && (e as Record<string, unknown>).kind === 'forOf',
			);
			const conditionalEvents = events.filter(
				(e) => e.category === 'controlFlow' && (e as Record<string, unknown>).kind === 'conditional',
			);
	...
	describe('event gating', () => {
		it('disabling controlFlow.events.test removes test events', async () => {
			const config = withOverride(ALL_ENABLED, 'controlFlow.events.test', false);
			const { events } = await drainGenerator(
				'let x = 5;\nif (x > 3) {\n  let y = 1;\n} else {\n  let y = 2;\n}\n', config,
			);
		...
		it('disabling controlFlow.events.branch removes branch events', async () => {
			const config = withOverride(ALL_ENABLED, 'controlFlow.events.branch', false);
			const { events } = await drainGenerator(
				'let x = 5;\nif (x > 3) {\n  let y = 1;\n} else {\n  let y = 2;\n}\n', config,
			);
			const branchEvents = events.filter(
		...
		it('disabling controlFlow.events.iteration removes iteration events', async () => {
			const config = withOverride(ALL_ENABLED, 'controlFlow.events.iteration', false);
			const { events } = await drainGenerator(
				'let i = 0;\nwhile (i < 2) {\n  i = i + 1;\n}\n', config,
			);
		...
		it('disabling controlFlow.events.jump removes jump events', async () => {
			const config = withOverride(ALL_ENABLED, 'controlFlow.events.jump', false);
			const { events } = await drainGenerator(
		...
		it('disabling controlFlow.events.do removes do events', async () => {
			...
			const doEvents = events.filter(
				(e) => e.category === 'controlFlow' && (e as Record<string, unknown>).event === 'do',
			);
			const testEvents = events.filter(
				(e) => e.category === 'controlFlow' && (e as Record<string, unknown>).event === 'test',
			);
		...
		it('disabling controlFlow.events.initialize removes initialize events', async () => {
			const config = withOverride(ALL_ENABLED, 'controlFlow.events.initialize', false);
			const { events } = await drainGenerator(
				'for (let i = 0; i < 2; i = i + 1) {\n  let x = i;\n}\n', config,
			);
			const initEvents = events.filter(
				(e) => e.category === 'controlFlow' && (e as Record<string, unknown>).event === 'initialize',
			);
			const testEvents = events.filter(
				(e) => e.category === 'controlFlow' && (e as Record<string, unknown>).event === 'test',
			);
		...
		it('disabling controlFlow.events.increment removes increment events', async () => {
			const config = withOverride(ALL_ENABLED, 'controlFlow.events.increment', false);
			const { events } = await drainGenerator(
				'for (let i = 0; i < 2; i = i + 1) {\n  let x = i;\n}\n', config,
			);
			const incrementEvents = events.filter(
				(e) => e.category === 'controlFlow' && (e as Record<string, unknown>).event === 'increment',
			);
			const testEvents = events.filter(
				(e) => e.category === 'controlFlow' && (e as Record<string, unknown>).event === 'test',
			);
	...
	describe('filter', () => {
		it('filter limits to named control flow structures', async () => {
			const config = withOverride(ALL_ENABLED, 'controlFlow.filter', ['conditional']);
			const { events } = await drainGenerator(
				'let x = 5;\nif (x > 3) {\n  let y = 1;\n}\nlet i = 0;\nwhile (i < 2) {\n  i = i + 1;\n}\n', config,
			);
			const conditionalEvents = events.filter(
	...
	describe('filter', () => {
		it('filter limits to named functions', async () => {
			...
			const addEvents = events.filter(
				(e) => e.category === 'function' && (e as Record<string, unknown>).name === 'add',
			);
			const subEvents = events.filter(
				(e) => e.category === 'function' && (e as Record<string, unknown>).name === 'sub',
			);
