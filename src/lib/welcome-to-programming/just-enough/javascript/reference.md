...
- [Key Concepts](#key-concepts)
  ...
  - [Validation Rejections & Warnings](#validation-rejections--warnings)
...
### Validation Rejections & Warnings

Programs in the curriculum check this language level when they run. Using
features that aren't on this page will fail validation — but validation only
checks that you're using allowed syntax. It won't catch runtime errors like
calling `.length` on a number. Those happen when the program runs.

**Validation Rejections** mean your code uses syntax or features outside JeJ. These
must be fixed before your program can run. Examples: using `var` instead of
`let`, calling `Math.random()`, writing a `function` declaration.

**Validation Warnings** mean your code is valid JeJ, but something looks off.
Your program can still run, but you should review the flagged code. Here's what
each warning means:

- **`'use strict'`** — Redundant. Modules are always in strict mode, so you
  don't need this directive.
- **Unused expression** — You wrote something like `5;` or `x + 1;` as a
  statement. The result isn't used anywhere — did you mean to assign it or call
  a function?
- **Non-camelCase name** — Variable names should use `camelCase` like
  `myVariable`, not `my_variable` or `MY_VAR`.
- **Empty block** — You wrote `if (...) {}` or `while (...) {}` with nothing
  inside the curly braces. Did you forget to add code?
- **Assignment in condition** — You wrote `if (x = 5)` which _assigns_ 5 to x.
  You probably meant `if (x === 5)` to _compare_.
- **Unreachable code** — Code after `break` or `continue` will never run.
- **Unused variable** — You declared a variable but never used it.
- **Variable shadowing** — An inner block re-declares a variable with the same
  name as one in an outer scope. This is almost always accidental.
- **for-of with `let`** — Use `const` for the iteration variable. It shouldn't
  be reassigned inside the loop.
- **for-of var reassigned** — You're reassigning the iteration variable inside
  the loop body. This is usually a mistake.
- **Missing semicolon** — A statement is missing its `;` at the end.
- **Unnecessary semicolon** — Extra `;` where none is needed, like `;;` or
  `if (...) {};`.
- **Tabs not spaces** — Use tabs for indentation, not spaces. Tabs are more
  accessible because each user can set their preferred visual width.
- **Trailing newline** — Files should end with exactly one newline character.
  Missing or extra blank lines at the end are flagged.

Think of rejections as guardrails that keep you inside JeJ, and warnings as gentle
