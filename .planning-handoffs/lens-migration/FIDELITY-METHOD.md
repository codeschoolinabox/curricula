...
| heading class  | matched by                                               | measured presence           |
| -------------- | -------------------------------------------------------- | --------------------------- |
| drops          | `grep -cE '^#+ What this lens does NOT do' README.md`    | 6 of 8                      |
| future         | `grep -cE '^#+ Future direction'` in README **and** DOCS | README 6 of 8 · DOCS 8 of 8 |
| out of scope   | `grep -cE '^#+ Out of scope' DOCS.md` (level 3)          | 8 of 8                      |
| naming         | `grep -cE '^#+ Naming' DOCS.md`                          | 1 of 8                      |
| named decision | `grep -cE '^## Why '` in README **and** DOCS             | README 1 each · DOCS 3–9    |
...
ledger opens with a `## Reference inventory` section — the complete heading list
of the reference documents plus the `types.ts` line count — pasted from:
...
reviewer's "what was skipped?" is a single pass down six labels rather than a
diff against a 27-heading list. The alternative — every row transcribing the
full heading list with the walked ones marked — multiplies transcription by the
row count, and mis-transcription is the failure this column already has on
record: `annotate-007` shipped `Why two views one lens` for a heading that reads
...
stale the way this campaign's register did. The rule: **any lens with no Gen-2
directory.** Measured today that is **nine** of the register's names —
`dropdowns`, `variables`, `print`, `run-javascript`, `debug-javascript`,
`trace-javascript`, `tables-universal`, `step-throughs`, `tracing` [measured
2026-08-14: `ls -d src/lib/study-lenses--deprecated-architecture/lenses/*/` →
annotate, blanks, debug-props, lib, parsons, quiz, socratize, trace-debugging,
writeme]. Six of those nine are Family F, so **`_family-f.md` is the largest
ledger this clause governs**, not an afterthought to `dropdowns` and
`variables`.
...
|                | **`walked`** — against `ledgers/blanks.md` § Reference inventory. **drops:** README § What this lens does NOT do. **future:** README § Future direction; DOCS § Future direction. **out of scope:** DOCS § Out of scope. **naming:** `empty` — blanks' DOCS has no such heading. **named decision:** README § Why this lens exists; DOCS §§ Why this module exists, Why preserve learner answers across view-mode toggle, Why per-blank feedback ships on, Why hints are orthogonal to difficulty, Why drop the seeded RNG, Why position-aware evaluation is in scope, Why drop URL config sync. **type contract:** `blanks/types.ts` read in full. **also walked** (the set is a floor, not a ceiling): README § Hints panel contract, where the ruling lives.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |                             |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |                                                                                                                                                                     |                                                                                                           |                                      |
|                | **`found`** — **five sections bear on this row and all five confirm the supersession.** README § Hints panel contract: _"ships a **cursor-scoped, on-demand, positional** hints panel per user-directed redesign. The legacy 3-tier system (`'auto' \| 'easy' \| 'medium' \| 'hard'` controlling rendered richness) is gone"_ and _The "tier" is now emergent — how many blanks the learner chooses to peek at across a session is itself the scaffolding gradient._ (italics are mine — no outer quote marks, because the source's own double quotes around "tier" would nest) DOCS § Why hints are orthogonal to difficulty: _"User-directed redesign rejected this coupling on pedagogical grounds: **the learner, not the slider, should control scaffolding**."_ README § What this lens does NOT do: _"**Hints panel ships ENABLED at parity.** Legacy compiled it out with `{false && showHints && (...)}` (line 672); the design and styling exist and are load-bearing pedagogically."_ DOCS § Why per-blank feedback ships on: _"the design and styling exist in full … v1 splits the two affordances the legacy conflated"_ and _"Disabling it in legacy was a ship cut, not a design decision"_ — so the panel returns while its 3-tier control does not, which is precisely what `supersede` means here. `types.ts` § the config type's `@remarks` Naming note: _"vocabulary matches the legacy `BlanksLens.jsx` directly (`blank`, `blankenated`, `content type`, `view mode`, `hints level`, `correctness`)"_ — **the term `hints level` outlived the mechanism it named**; the greenfield must not silently reuse it for the emergent ladder. Not rebutted anywhere — the row yields. Remaining walked sections bear nothing. |                             |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |                                                                                                                                                                     |                                                                                                           |                                      |
...
**Open rows at AR-5 must be zero.** A row is open when its `discharged by` cell
is empty, or when a filled cell does not resolve.
...
a license for tomorrow's.** If a boundary row is ever written with disposition
`revive` or `ADDITION`, it carries `walked` and `found` like any other row.
...
[SPEC.md § The register check](../SPEC.md#the-register-check): every lens name
the three trees and the wired roster produce has exactly one row in SPEC's
register. It runs at Gate 1 as well, because the register goes stale between
drafts rather than between campaigns.
