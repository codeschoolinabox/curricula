...
<!-- cspell:ignore parameterised normalisation -->
...
- **Glossary literal-presence counts in the port**, evidence for Pass 2 and not
  ...
  [measured 2026-08-16].
- **One instrument caveat on `glossterm` itself**, recorded because it changed
  which cells below are trustworthy: **the `glossterm` published when rows
  `034`–`045` were cut matched its term as an awk regex, so a term containing
  `(` or `)` never matched.** `Diff (toggle)` returned nothing on **both** the
  reference and the port, which would read as a definitional loss and is not
  one. **`e8f81de8` fixed the helper to match a literal prefix, and today's
  published form returns the bullet on both sides** [measured 2026-08-17]. This
  is the same defect class FIDELITY-METHOD § The minimum walk set already
...
**Every quotation below is now the published extractor's own output, and that is
a re-run rather than a claim.** Run
[`_TEMPLATE.md` § The transport check](./_TEMPLATE.md#the-transport-check)
against this file: **0 divergent, 1 unquoted** [measured 2026-08-17]. Against
this ledger at `0f9257c8` the same command reports **14 divergent halves across
12 rows and 11 unquoted rows**. The one survivor is `writeme-019` and it is
accounted for below.

**Do not take those numbers from this paragraph.** The command is published, it
is parameterised by nothing but three paths, and a reader who runs it gets the
current answer rather than yesterday's. That is the whole reason it is published
— an earlier revision of this very paragraph carried a pair nobody else could
reproduce, because the normalisation that produced it lived only in the author's
shell.

**Twelve rows, not thirteen, and the difference is a rule rather than a
correction.** `0f9257c8` measured **13** hand-cut cells and **that measurement
stands**; the check it used had no concept of a sanctioned transport
modification. Under the five `_TEMPLATE.md` now names, `writeme-006` is no
longer among them — its `\[…\]` escaping is modification 5, forced rather than
authored. Same rows, one of them reclassified by a rule that did not exist when
it was counted.

**Eleven rows were unquoted, which is a different defect the earlier check could
not see.** Each cited a Gen-3 section and gave no quotation, so there was
nothing for a stored-quote diff to compare;
[§ Columns](../FIDELITY-METHOD.md#columns) requires a quoted fragment **for
every provenance tag**. Three carried a prose delta (`005`, `007`, `022`) and
eight asserted _"the same opening block, verbatim"_. Every one of those claims
was true — the injected quotations were re-verified by re-running the extractor
on both sides, and none diverges. Two checks, two questions, two correct
answers.

**Two rows are formatter-forced and neither is under-transport:**

- **`writeme-006`** carries `\[Diff semantics\](#diff-semantics)` because
  unescaping it makes `MD051/link-fragments` fire — this ledger has no
  `## Diff semantics` heading for the fragment to resolve against [measured
  2026-08-17: unescaped → **1** MD051 error; escaped → **0**]. Rows `011` and
  `015` survive unescaped because their links are **paths**, not fragments.
- **`writeme-028` is quoted in `<em>` rather than `_…_`.** Its Gen-2 block holds
  a code span containing an underscore, which breaks prettier's emphasis pairing
  for the rest of the cell and elides the spaces around every later code span —
  so a verbatim `_…_` cell is re-damaged on the next format. `<em>` round-trips
  byte-identically and `em` is already on `MD033`'s allowed list [measured
  2026-08-17].

**`writeme-019` is the one unquoted row, and it cannot be otherwise.**
`firstblock` returns empty against the port's § Architectural sketch, because
that heading is followed immediately by its first child heading. There is no
quotation to give, the row says so in an instrument caveat, and the check
reports it rather than being taught to ignore it — **a special case in the
instrument would be worth less than a reported one in the ledger.**

`writeme-011` remains the sharpest illustration of the original defect:
`firstblock` on the port's § Two-layer module returns a complete
**240-character** block ending in `:` and makes **no cut at all** —
`length(buf) > 240` is false at exactly 240 — while the shipped cell showed a
truncated sentence plus a `…` the tool never produced. **The glossary rows'
Gen-2 halves were clean throughout: 0 of 12 diverge.** Nothing quoted anywhere
was fabricated and no row cites a heading that does not resolve [both re-checked
2026-08-17].

⚠️ **The glossary rows' Gen-3 halves are still owed a quotation.** Eleven of
them carry a `G3` tag whose evidence is the prose assertion _"still defined in
the port's `## Glossary`"_ — the same gap `005`/`007`/`022` just had, one seed
class over. They are not citation-shaped, so the transport check does not reach
them; that is a limit of the check, not a clean bill. **Owed to Pass 2**, named
...
| `writeme-002` | A reader can find out what this lens asks of the learner, and how that differs from the two lenses that scaffold more.                  | `G2-doc` + `G3` + UNSETTLED | Gen-2 `README.md` § Why this lens exists: _"`writeme` is the learner's **reproduction workbench**. Where `parsons` asks the learner to order given lines and `blanks` asks them to fill given holes, `writeme` asks them to **produce the whole program from memory** — the furthest …"_ · Gen-3 `README.md` § Why this lens exists: _"`writeme` is the learner's **reproduction workbench**. Where `parsons` asks the learner to order given lines and `blanks` asks them to fill given holes, `writeme` asks them to **produce the whole program from memory** — the furthest …"_ — byte-identical to the Gen-2 block, re-verified by re-running the extractor on both sides.                                                                                                                                                                                                                                                                                      |             |               |      |
...
| `writeme-013` | A reader can find what this lens defers, and the restoration path for each deferral.                                                    | `G2-doc` + `G3` + UNSETTLED | Gen-2 `README.md` § Future direction: _"- **WS2 `recommend()`.** Lens ships with `recommend: () => []`. Once WS2's analysis surface lands, `recommend(embodiment)` populates Block-Model placements (and down-ranks snippets too short to be worth retyping). - \*\*Order-insensitive …"_ · Gen-3 `README.md` § Future direction: _"- **`recommend()` heuristics.** The lens ships with `recommend` returning the frozen empty array; a future analysis surface can propose writeme for snippets worth retyping (and down-rank ones too short). - \*\*Order-insensitive line …"_                                                                                                                                                                                                                                                                                                                                                                                     |             |               |      |
...
| `writeme-015` | A reader can reach every sibling document of this lens from its README.                                                                 | `G2-doc` + `G3` + UNSETTLED | Gen-2 `README.md` § Navigation: _"- **Parent**: [`../README.md`](../README.md) — lenses peer. - **Architectural sketch**: [`./DOCS.md`](./DOCS.md). - **Type contract**: [`./types.ts`](./types.ts). - **Lens contract**: [`../types.ts`](../types.ts) — `LensModule` + …"_ · Gen-3 `README.md` § Navigation: _"- **Region**: [`../README.md`](../README.md) — the lens kind's mechanics. - **Architecture & decisions**: [`./DOCS.md`](./DOCS.md). - **Type contract**: [`./types.ts`](./types.ts). - **Lens-kind contract**: [`../types.ts`](../types.ts) …"_                                                                                                                                                                                                                                                                                                                                                                                                      |             |               |      |
| `writeme-016` | A reader can find out what the module is for, in the learner's terms, before reading any of its structure.                              | `G2-doc` + `G3` + UNSETTLED | Gen-2 `DOCS.md` § Why this module exists: _"The `writeme` lens is the learner's **reproduction workbench**: a place to type a program back from memory into a paste-blocked CodeMirror editor, with an optional comment skeleton as scaffolding and a per-line diff **pair** as the …"_ · Gen-3 `DOCS.md` § Why this module exists: _"The `writeme` lens is the learner's **reproduction workbench**: a place to type a program back from memory into a paste-blocked CodeMirror editor, with an optional comment skeleton as scaffolding and a per-line diff **pair** as the …"_ — byte-identical to the Gen-2 block, re-verified by re-running the extractor on both sides.                                                                                                                                                                                                                                                                                        |             |               |      |
...
| `writeme-018` | A reader can find a per-file table naming each module's layer and purpose.                                                              | `G2-doc` + `G3` + UNSETTLED | Gen-2 `DOCS.md` § Modules: _"\| File \| Layer \| Purpose \|"_ (a table; the extractor takes its first line, whose column padding prettier collapses) · Gen-3 `DOCS.md` § Modules: _"\| File \| Layer \| Purpose \|"_ — heading survives on both sides, and the two first lines are identical after that collapse.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |             |               |      |
...
| `writeme-022` | A reader can find the constraints the implementation must not violate, stated separately from what it does.                             | `G2-doc` + `G3` + UNSETTLED | Gen-2 `DOCS.md` § Structural constraints: _"- **Two-layer module shape** — `core.ts` + the files under `lib/` do NOT `import React`. Three lib files import `@codemirror/*` (a third-party library whose extension types are React-free): `no-paste-extension.ts`, …"_ · Gen-3 `DOCS.md` § Structural constraints: _"- **Two-layer module shape** — `core.ts` + the files under `lib/` do NOT import React. Three lib files import `@codemirror/*` (a third-party library whose extension types are React-free): `no-paste-extension.ts`, …"_                                                                                                                                                                                                                                                                                                                                                                                                                        |             |               |      |
| `writeme-023` | A reader can find what the lens deliberately leaves to another owner, separately from what it defers.                                   | `G2-doc` + `G3` + UNSETTLED | Gen-2 `DOCS.md` § Out of scope: _"- **Cross-mount persistence** of learner code. Per-mount React state only; nothing survives unmount. - **URL state.** v1 ships none. A Future-direction item; would lift to the orchestrator per the blanks precedent. \*\*Deliberate …"_ · Gen-3 `DOCS.md` § Out of scope: _"- **Cross-mount persistence** of learner code. Per-mount React state only; nothing survives unmount. - **URL state.** None; persistence of study settings is orchestrator-domain. - **Program mutation / editing.** The lens is a read-only …"_                                                                                                                                                                                                                                                                                                                                                                                                      |             |               |      |
| `writeme-024` | A reader can find out why toggling a scaffold swaps an editor extension instead of rebuilding the editor.                               | `G2-doc` + `G3` + UNSETTLED | Gen-2 `DOCS.md` § Why scaffold toggles use compartments: _"The four scaffold toggles (colorize / suggestions / comments / diff) each swap an editor extension on or off. The naive implementation — list the toggle states in the mount-effect deps and rebuild the `EditorView` on every change — …"_ · Gen-3 `DOCS.md` § Why scaffold toggles use compartments: _"The four scaffold toggles (colorize / suggestions / comments / diff) each swap an editor extension on or off. The naive implementation — list the toggle states in the mount-effect deps and rebuild the `EditorView` on every change — …"_ — byte-identical to the Gen-2 block, re-verified by re-running the extractor on both sides.                                                                                                                                                                                                                                                          |             |               |      |
| `writeme-025` | A reader can find out why the learner sees feedback on first paint rather than having to switch it on.                                  | `G2-doc` + `G3` + UNSETTLED | Gen-2 `DOCS.md` § Why default the editor to diff: _"The `diff` toggle defaults ON (feedback on), not off. A feedback-off default is a feedback-free box — the precise shape of the weak-shell failure this redo exists to prevent (per `./README.md` § Why this lens exists). Defaulting `diff` …"_ · Gen-3 `DOCS.md` § Why default the editor to diff: _"The `diff` toggle defaults ON (feedback on), not off. A feedback-off default is a feedback-free box. Defaulting `diff` on makes the honest-feedback mechanism visible on first paint: the learner sees the lens working, and the diff guides …"_                                                                                                                                                                                                                                                                                                                                                           |             |               |      |
| `writeme-026` | A reader can find out why the read view shows the solution alone rather than the learner's code beside it.                              | `G2-doc` + `G3` + UNSETTLED | Gen-2 `DOCS.md` § Why Read is the paired solution editor: _"The read view shows the solution in a **read-only editor configured to match the write editor** — a pair — not the learner's code beside it. The learner's typed code is never shown in Read; only the solution (mirroring `colorize`, and, …"_ · Gen-3 `DOCS.md` § Why Read is the paired solution editor: _"The read view shows the solution in a **read-only editor configured to match the write editor** — a pair — not the learner's code beside it. The learner's typed code is never shown in Read; only the solution (mirroring `colorize`, and, …"_ — byte-identical to the Gen-2 block, re-verified by re-running the extractor on both sides.                                                                                                                                                                                                                                                |             |               |      |
...
| `writeme-028` | A reader can find out why the diff compares whole lines rather than characters.                                                         | `G2-doc` + `G3` + UNSETTLED | Gen-2 `DOCS.md` § Why per-line (not per-char) diff: <em>"The `blanks` diff highlights per-character mismatches, which works only because its blanked source is **length-matched** to the original (each blanked token is replaced by an equal-length run of `_`), so character offset `i` in the …"</em> · Gen-3 `DOCS.md` § Why per-line (not per-char) diff: <em>"A per-character positional diff works only when the learner's document is **length-matched** to the original, so character offset `i` in the learner's document corresponds to offset `i` in the original. writeme's starting template is the …"</em> ⚠️ Instrument caveat: both quotations are wrapped in `<em>` rather than `_…_` because the Gen-2 block carries a code span holding an underscore, which breaks prettier's emphasis pairing for the rest of the cell and silently elides the spaces around every later code span; `<em>` round-trips byte-identically [measured 2026-08-17]. |             |               |      |
| `writeme-029` | A reader can find out what the keep-comments toggle does once the learner has started typing.                                           | `G2-doc` + `G3` + UNSETTLED | Gen-2 `DOCS.md` § Why keep-comments re-seeds only while pristine: _"The keep-comments toggle selects the starting template (comment skeleton vs blank slate). The question is what it does mid-exercise. The legacy guarded its student-editor seed effect with `studentEditorInitialized` (legacy lines 194 / …"_ · Gen-3 `DOCS.md` § Why keep-comments re-seeds only while pristine: _"The keep-comments toggle selects the starting template (comment skeleton vs blank slate). The question is what it does mid-exercise. Two failure modes bound the design: a dead control (a toggle that visibly does nothing once typing began) …"_                                                                                                                                                                                                                                                                                                                          |             |               |      |
| `writeme-030` | A reader can find out why flipping any scaffold leaves the learner's typed code, cursor and history intact.                             | `G2-doc` + `G3` + UNSETTLED | Gen-2 `DOCS.md` § Why toggling a scaffold preserves learner code: _"Every scaffold toggle — colorize, suggestions, comments, diff — leaves the learner's typed code, cursor, and history intact, because none of them remounts the editor. The editor mounts once (mount-effect deps `[]`); colorize / …"_ · Gen-3 `DOCS.md` § Why toggling a scaffold preserves learner code: _"Every scaffold toggle — colorize, suggestions, comments, diff — leaves the learner's typed code, cursor, and history intact, because none of them remounts the editor. The editor mounts once (mount-effect deps `[]`); colorize / …"_ — byte-identical to the Gen-2 block, re-verified by re-running the extractor on both sides.                                                                                                                                                                                                                                                  |             |               |      |
...
prose, so an unscoped `grep -c 'UNSETTLED'` returns **49** against 45 rows and
the banner counts **2** [measured 2026-08-17, this ledger, after the re-cut].
Row-scoped, all eight checks pass: 45 rows, 45 carrying `UNSETTLED`, banner
once, zero `—` cells, zero disposition values, zero
`walked`/`found`/`Design owed`, ids `001–045` with no gap or duplicate.

**The unscoped number moved from 47 to 49 and the row count did not.** That is
the drift this bullet exists to make harmless: the re-cut added two prose
mentions of the marker and touched no row. A session that transcribed the old 47
rather than re-running the count would have published a number that was true
yesterday — which is exactly how a whole-file grep gets trusted in the first
place.
