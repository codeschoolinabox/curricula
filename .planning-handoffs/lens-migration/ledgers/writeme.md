...
`…` marks the cut in both extractors. `\|` is `firstblock`'s pipe escape. No row
below comes from lister 4 or lister 5: both measured zero.
...
prose, so an unscoped grep over-counts both.
