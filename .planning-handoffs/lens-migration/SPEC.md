...
**This file supersedes `src/lib/study-lenses/lenses/MIGRATION-PLAYBOOK.md`,
which this campaign has deleted.** Two control panels is how stale facts
survive: the playbook told agents that `built-in-lenses.ts` was empty when it
had held three lenses since `47234d7c`. Its live content is transported below;
every omission is enumerated in [ledgers/\_playbook.md](./ledgers/_playbook.md),
the campaign applying its own instrument to itself. The source is retired, not
lost — recover it with
`git log --diff-filter=D -- 'src/lib/study-lenses/lenses/MIGRATION-PLAYBOOK.md'`
and then `git show <that sha>^:<that path>`.
...
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Gen 1 — ORIGINALS** | `/Users/master/Documents/0-teach-code/0-spiralearn/0-study-lenses-committee/zz--oldd-clauding-and-context-dump/0--study-lenses--it-begins/` — **two roots, `src/lenses/` and `public/static/`** (human ruling 2026-08-15) | **Visual and pedagogical intent.** Preact, flat `XxxLens.jsx` + `XxxLens.module.css` pairs. 12 render lenses + 6 action lenses. **The 30 lens files total 14,075 lines** — 18 `.jsx` at 8,161 plus 12 `.module.css` at 5,914; the 14,377 an earlier revision published additionally counted `index.js` (179) and `__tests__/unified-api.test.js` (123), which are not lenses [all measured 2026-08-14: `wc -l`]. Rich CSS; much of its best design never shipped. **But those 30 files are frequently only the shell — see below.** |
...
### Gen 1's second root — the lens file is often only a shell

**Human ruling 2026-08-15**, taken after the first seeded ledger reported 47
rows with **zero `G1-live`** while three of its own rows quoted files it had not
...
`run-javascript`, `debug-javascript` [measured 2026-08-15: `grep -lE 'iframe'`].
So "the Gen-1 lens file" is routinely **not** where the lens lived, and R-2's
union targets behaviours — a shell is not the behaviour.

Two consequences worth stating so they are not rediscovered:

- **`dist/` is used for nothing.** It is a build output of `public/` and carries
  the same parsonizer file set [measured 2026-08-15].
...
- **`quiz` and `socratize`** belong to the socratize-quiz re-enrichment campaign
  (canon:
  [`.planning-handoffs/socratize-quiz-reenrichment/SPEC.md` § The un-colorized ruling record (R-4)](../socratize-quiz-reenrichment/SPEC.md#the-un-colorized-ruling-record-r-4)).
  Do not port them and do not rule for them. Two of their rulings reach into
  this campaign: **socratize stays un-colorized** (their R-4) so a colorize-all
  sweep skips it, and **quiz's coloring is deferred to their own lens-build
  time** (their R-4a) — this campaign publishes a consumable vocabulary and
  records the skip.
...
a note.** Retiring `MIGRATION-PLAYBOOK.md` broke a live citation: the
socratize-quiz campaign's SPEC cited
...
socratize exception verbatim. **This campaign has re-pointed that citation
here**, because the break was its own act. Re-pointing is not the same as being
held: the recipient still has to acknowledge `bnd-002`, and by this campaign's
own rule an unacknowledged boundary row is an OPEN row at close.
...
| **`spellme`**                                                                                                               | **out of scope, and a live coordination boundary.** A concurrent session built it and **committed it** — `lenses/spellme/`, 9 tracked files including a `ux/` twin directory [measured 2026-08-14: `git ls-files`], a `tokens`-phase "drive-the-scanner" exercise, not yet in `built-in-lenses.ts`. No Gen-1 and no Gen-2 source, so nothing to migrate. **But it renders classed code elements and names two semantic roles — attested element, diverging claim — while deliberately leaving their hues to this package** (human ruling 2026-08-14, in that lens). That is a third surface bound by the coloring foundation's palette choice, and its author has already deferred to it. Carried as `bnd-009`. | —      |
| **`lib/scanning`**                                                                                                          | **not a lens** — a sibling library of `lib/classifying`, created the same night and now **tracked**, 5 files [measured 2026-08-14: `git ls-files`], deriving an element sequence from the tokens fact for `spellme`. It matters here because it is a **third `facts.tokens` derivation on the tier `lib/colorizing` will land in**, and the foundation's Phase 0 owes that seam a look.                                                                                                                                                                                                                                                                                                                         | —      |
...
# cspell:ignore lines stripped FIRST -- this check's one real hole, see below.
# Flattened after, because prettier wraps and a line-based grep misses a name.
FLAT=$(sed '/cspell:ignore/d' "$SPEC" | tr '\n' ' ')
{
  ...
  if [ -d "$GEN1" ]; then ls "$GEN1"/*.jsx | xargs -n1 basename | sed 's/\.jsx$//'
  else echo "ARM 3 SKIPPED -- Gen-1 tree not mounted; PARTIAL run" >&2; fi
...
**What it does not do, stated so nobody reads it as total:**
...
- **It matches anywhere in the file, so the `cspell:ignore` lines come out
  first.** Six lens names sit in this file's ignore comments, and a name deleted
  from the register survived there — the check passed on a SPEC carrying no
  register row for it [measured 2026-08-14, before the `sed` was added]. Not an
  incidental hole: [RESUME.md](./RESUME.md) § Operating instructions directs
  every session to add genuinely-new terms to exactly that line, so **the
  convention masked precisely the new lens names this check exists to catch.**
  The six names are deliberately not repeated in this paragraph — writing them
  here would recreate the mask one section further down.
- **A region-scoped variant does not work in this file.** An intermediate
  revision extracted a `<!-- register:start/end -->` region; the marker inside
  the command's own code fence re-opened the `sed` range, so it read **876 of
  1120 lines** instead of 21 [measured 2026-08-14]. **A check embedded in the
  document it checks must not match on text it itself contains** — that is the
  general lesson, and it cost two revisions.

Arm 3 is unavailable when the `0-study-lenses-committee` tree is not mounted.
**A bare `[ -d ] &&` guard is not enough** — it swallows the failure, and the
check then prints nothing and exits 0, which is byte-identical to success. The
`else` branch is what makes an unmounted tree a _reported_ partial run. An
earlier revision had the guard without the branch **and** wrote the path as a
literal `…` ellipsis, so the published command silently checked 9 names instead
of 27 [measured 2026-08-14 by a cold reader running it verbatim]. That is the
third time this campaign has published a check that reports success over nothing
— hence the `else`, and hence the full path inline rather than an elision.
...
`spellme` lens **names** those two roles — attested element, diverging claim —
and deliberately **does not bind them to hues**, deferring the pair to this
foundation (human ruling 2026-08-14, in that lens).
...
flowchart TD
  subgraph T0["TIER 0 — establishment"]
    ...
    HAND["5 handoffs, each context-free validated"]
    SPEC --> SEED --> HAND
  end
  ...
  PF["F · evaluator-gated — LEDGER ONLY, no handoff"]
  G2 --> PA & PC
  G1 --> PF
...
| The **`[read: MIGRATION-PLAYBOOK.md locked decision (1)]` citation** in `.planning-handoffs/socratize-quiz-reenrichment/SPEC.md` | socratize-quiz campaign                                                                                                        | This campaign deleted the cited file and **has already re-pointed the citation** at [Standing exclusions](#standing-exclusions) above. What remains owed is the acknowledgement: the recipient must know that decision (1)'s **mechanism** is superseded by R-1 while its **socratize exception is restored verbatim**. Created by this campaign's own act, so it is an obligation rather than a note. |
...
- **Ledger open rows = 0**, under
  [FIDELITY-METHOD.md § At AR-5](./FIDELITY-METHOD.md#at-ar-5)'s single
  four-part definition — which this does not restate. **Every** row carries a
  filled, resolving `discharged by`, on any disposition (human ruling
...
a different lens and a Family F member]. Listers 4 and 5 read a Gen-1 source
only, so neither can run.

**Family F is mixed, and one member has no runnable lister at all.** A single
cell cannot carry it [all measured 2026-08-14: `ls "$GEN1"` for the Gen-1 files;
`ls src/lib/study-lenses/lenses/` for the port]:

| Family F member                                                              | instruments | why                                                                                                        |
| ---------------------------------------------------------------------------- | ----------- | ---------------------------------------------------------------------------------------------------------- |
| `step-throughs` (`StepThroughsLens`), `tracing` (`TracingLens`)              | **4,5**     | Gen-1 `.jsx` **and** `.module.css`; no Gen-2, no Gen-3                                                     |
| `run-javascript`, `debug-javascript`, `trace-javascript`, `tables-universal` | **5**       | Gen-1 `.jsx` only — **no `.module.css`**, so lister 4 has nothing to read                                  |
| `trace-debugging`                                                            | **none**    | Gen-2 directory only: no Gen-1 source for listers 4–5, and no Gen-3 port and no Gen-1 side for listers 1–3 |

**`trace-debugging` seeds entirely from Pass 2** — a whole-file read of its
Gen-2 documents — and its ledger says so rather than reading as a thin result.
An earlier revision of this table had it exactly backwards, listing it as the
one Family F member with all five.
...
| Gen-1 originals (READ-ONLY, separate tree) | **Two roots** under `/Users/master/Documents/0-teach-code/0-spiralearn/0-study-lenses-committee/zz--oldd-clauding-and-context-dump/0--study-lenses--it-begins/` — `src/lenses/` (the shells) **and** `public/static/` (the engines), human ruling 2026-08-15; see [Gen 1's second root](#gen-1s-second-root--the-lens-file-is-often-only-a-shell). Ignore `dist/`. |
