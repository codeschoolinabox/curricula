...
| F4  | **Module homes for the copy** — the display-labels entry enumerates the keyed-or-derived families; the manifest names **one** file | `README.md` § What lives here | `README.md` glossary · display labels; `display-labels.ts`, which holds one string per phase and has a live consumer [read: `display-labels.ts`; measured: `index.tsx:654`]; rows C1, C2, C3, C5, C9, C11, C12, C13                                                                                                                                                                                                                          | **SETTLED** — `display-labels.ts` is the copy's one home (human ruling 2026-08-18); `README.md` § What lives here now enumerates the families it owns and states why a `copy/` directory was declined. **The widening itself is 0.3's** (C11 — the file has a live consumer at `index.tsx:654`) |
...
**The `strip` residue after I6's discharge is DECLARED, per receipt-rule
amendment 2** — a stated non-receipt rather than a silent survivor, so round 9
meets a recorded decision:

| surviving site                                                                                         | why it is not the enumeration                                                                                                                                                             |
| ------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ux/wireframes.md` ×2 — "a lens is closed either by the Edit code button or by the strip's none entry" | **Sanctioned lineage.** It names what the arrangement replaces, and must keep the retired word to be legible at all — the same category as the glossary's "Deliberately not 'the break'". |
| `README.md` · `DOCS.md` — "inert and dimmed while masked, **like the strip's selects**"                | A class-3 TREATMENT comparison, not a dispose raiser. Its analogue under the Rail is the trays, and swapping it is the vocabulary half.                                                   |
| `README.md` § The host surface — "the lifecycle strip and every control label are inline text"         | Vocabulary (human ruling 2026-08-18). A claim about heading structure, not about dispose. **Note it also sits in G4's home of record**, which G6 deliberately did not open.               |
| `README.md` § What renders — "the band, the strip, the level UI … all stay rendered"                   | A render list naming the surface, not the enumeration.                                                                                                                                    |
| `generator/README.md` — "`Evaluation · run` is the lifecycle strip's own display label"                | Asserts the strip OWNS a display label the rail now draws. Borderline; called as vocabulary rather than left silent.                                                                      |
