...
| C13 | **The barred cause line's shape** — one authored constant, or keyed by the stage that failed?                                           | `README.md` glossary · display labels               | `ux/wireframes.md` § The parse breaks (the `ast` framing, drawn) and **§ The spelling breaks (the `tokens` framing, drawn here for the first time)**; `README.md` glossary · the barring edge, which uses the framing as its worked example for C6; **`ux/wireframes.md` § What the region is handed** — the projection contract, which asserted every cause is "the parser's message" (round 9 IMPORTANT 4) | **SETTLED** — THREE authored framings, keyed by the stage that FAILED: `tokens` → _the spelling broke here_, `ast` → _the grammar broke here_, `entwined`/`environment` → _the machinery broke here, not your code_. The third is now AUTHORED rather than described (round 9 IMPORTANT 4)                                  |
...
| **C11** · the short labels' value shape             | C11                        | `display-labels.ts` carries **one** string per phase; the contract says two. It has a live consumer (`index.tsx:654`) — the contrast with the `SurfaceClass` rename, which shipped at once because it had none.                                                                                                                                                                                                                                                                                                                                                             |
