...
| C13 | **The barred cause line's shape** — one authored constant, or keyed by the stage that failed?                                           | `README.md` glossary · display labels               | `ux/wireframes.md` § The parse breaks (drawn: `the grammar broke here — Unexpected token (2:8).`); `README.md` glossary · the barring edge, which uses the same string as its worked example for C6                                                               | **open (round 8 BLOCKER 3)** — the constant is FALSE whenever the origin is `tokens`; nothing reached the grammar      |
