...
| A2  | The class-2 **roster**, and the routes by which a node earns the class                 | `README.md` § Enforcement                           | all four A1 sites, plus `DOCS.md` § Structural constraints · Class-2 nodes never mask; `ux/wireframes.md` § What the arrangement never changes; **`guide/README.md`**; **`level-ui/README.md`**             | **open (R-D, R-E)** — **eight** homes, not four                          |
| A3  | The **rail is class 3**, argued from **exhaustion** (not lineage, not containment)     | `ux/wireframes.md` § Strict, covering — editor mode | `README.md` glossary · the rail                                                                                                                                                                             | open — A2 rewrites its premise                                           |
| A4  | The **nameplate is class 2**, by a third earning route — _naming where the learner is_ | none yet — R-D's interim home is `RESUME.md`        | nothing. **Zero of ten nameplate mentions assign a class** [measured: 5 in `README.md`, 5 in `ux/wireframes.md`]                                                                                            | **open (B3 / R-D)**                                                      |
| A5  | The **announcer is class 2**, the one member that is not a control                     | `README.md` glossary · announcer                    | `README.md` § Enforcement; `lib/masking/README.md`; `lib/masking/types.ts`; `DOCS.md` § Structural constraints; `ux/wireframes.md` § The parts, § Appendix (the override, Journey 6 bullet)                 | settled                                                                  |
...
| A9  | The **reason line beneath the rail is class 3** and dims with it — an unpaid cost      | `ux/wireframes.md` § Fresh mount                    | `ux/wireframes.md` § Strict, covering — editor mode (drawn annotation)                                                                                                                                      | **open (NEW)** — grounded on containment, which A6 forbids               |
| A10 | `SurfaceClass`'s class-2 literal `'meta-control'` **under-names the class**            | `lib/masking/types.ts` `SurfaceClass` JSDoc         | nothing — **one consumer, its own declaration** [measured: `git grep -n "SurfaceClass" -- src/` → 1 hit]                                                                                                    | open (M1) — one line today                                               |
| A11 | **Where the nameplate renders relative to the maskable containers**                    | none — no document states it                        | the announcer HAS this rule (A5, E3); the nameplate's ruling paragraph gives it a **different** warrant entirely — "the pane occupant is the top component's own state"                                     | **open (NEW)** — class 2 with no placement rule is unenforceable         |
| A12 | The announcer is "**the only member** of that class that is not a control"             | `README.md` glossary · announcer                    | `ux/wireframes.md` § The parts — "the one member of that class that is not a control"; `lib/masking/types.ts` — "admits **one node** that is not a control"                                                 | **open (NEW)** — R-D falsifies it: the nameplate is not a control either |
...
| B5  | A station with nothing to open has **no tray and no disclosure control** — not disabled | `README.md` glossary · tray                              | `ux/wireframes.md` § A station's tray open, § The kit at 0 1 and many, § The parts                                        | settled                          |
...
| B7  | The kit's size **never perturbs** the lifecycle geometry                                | `ux/wireframes.md` § The kit at 0 1 and many             | `ux/wireframes.md` § Fresh mount, § Appendix (the Rail); sandbox checkpoint T4                                            | settled                          |
...
| C6  | Display copy carries **no machine token and no coined term**                                 | `README.md` glossary · display labels               | `README.md` glossary · the barring edge (two registers)                                                                                 | **open (I5)** — falsified by `glossary · station`'s "reclaimed rather than minted" |
...
| C8  | The blocked sentence's ordering — **fix the code first, lift the guardrail last**            | `ux/wireframes.md` § Strict, covering — editor mode | **nowhere else.** `README.md` claims copy ownership and states no ordering                                                              | **open (I3)** — split homes                                                        |
| C9  | The **nameplate's two forms** (`your code` / `the pane holds: …`)                            | none — drawn only                                   | `ux/wireframes.md` § Fresh mount, § Strict covering with a lens open, § An excursion open, § The generator                              | **open (I4)** — no rule for which applies                                          |
| C10 | The README's copy inventory vs the copy the twin draws                                       | `README.md` glossary · display labels               | `ux/wireframes.md`'s "the region README owns every learner-facing string" **overreaches** — that phrase is the twin's, not the README's | **open (I4)**                                                                      |
...
| D8  | § What renders still specifies the **STRIP's** behaviour, not the Rail's                           | `README.md` § What renders             | nothing cites it — which is why no grep for `strip` or `rail` reaches it                                                                 | **open (B4)** |
...
| E5  | Where the announcer mounts                                                                       | settled by R-A                                       | `ux/wireframes.md` § What this document is asking, **item 3 still asks the review to settle it**                  | **open (I2)** — orphan |
...
| F3  | `phases-panel/` is gone from the manifest — **and still exists on disk** | —                             | zero manifest references [measured: `grep -oiE 'phases-panel'` on `README.md` → 0] while the directory is present [measured: `ls`] | **unfiled** → 0.3    |
...
| #   | decision                                                                                                             | home of record                                          | also asserts                                                                                                                                                                           | status                                                                                                                                                           |
| --- | -------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| G1  | The **band** holds everything rendered that is not the program: the control row and the rail                         | `README.md` glossary · band                             | `ux/personas.md` § The claim this document exists to make                                                                                                                              | **open (I7)** — the nameplate is rendered, is not the program, sits above the pane, and is neither                                                               |
...
| G5  | `DOCS.md` places the strip "**beside** the control row"; `README.md` puts the control row at the **top of the band** | conflict                                                | `DOCS.md` § The render projection vs `README.md` glossary · control row, band                                                                                                          | **open (M4)**                                                                                                                                                    |
...
| H3  | `kit` is **scoped by what precedes it** — unqualified means the whole roster                                                                                            | `README.md` glossary · kit              | `ux/wireframes.md` § The kit at 0 1 and many (**heading breaks the default**); `ux/personas.md` § The Frogrammer, § The reader who does not point | **open (M3)**                    |
...
| I5  | The posture argument holds "a second time and **independently**"                           | `ux/wireframes.md` § The parts                    | `0c78c63c`'s commit body asserted the one-way dependency             | **open (I1)** — the shipped text has no such independence |
...
| `SurfaceClass`'s `'meta-control'` rename                | A10    | free today (one consumer); a codemod after 0.3                                                                                                                                                                                                   |
...
## Open for the human — R-D cannot land until these are answered

Three things surfaced when this list was first used against the tree, and none
is the agent's to settle.

1. **R-D's route wording is falsified by the twin.** The ruling's substance —
   the nameplate takes class 2 by a third route — stands and is not being
   re-litigated. But the route **as worded**, _naming where the learner is_, is
   satisfied by the rail (see § A above), which would put the rail in class 2
   and destroy A3. The narrowest wording that still excludes the rail is
   **naming the pane's occupant**. That is a change to a ruling's words, so it
   is the human's.
2. **R-D opens A11 — the nameplate has no placement rule.** The announcer's
   class 2 is enforceable because a rule says where it renders relative to both
   maskable containers. The nameplate's ruling gives it a different warrant
   entirely, and class 2 with no placement rule is **the exact pre-`929d9086`
   defect one element over** — which is what B3 was.
3. **R-D's scope has grown from four homes to eight, plus A11 and A12.** A rule
   amendment ships alone [read: `DEV.md` § Atomic Commits], so the question is
   whether A11 and A12 ride the amendment (they are consequences of it) or land
