...
| A3  | The **rail is class 3**, argued from **exhaustion** (not lineage, not containment)                               | `ux/wireframes.md` § Strict, covering — editor mode | `README.md` glossary · the rail                                                                                                                                                                                          | **OPEN — round 6 B1.** `b9a534c7` re-derived it in `ux/wireframes.md` and MISSED `README.md` glossary · the rail, which this row named. Marked settled in error by `c331bf5f` |
...
| C11 | **The five short labels** — authored, or derived from the label?                             | none — `display labels` does not carry them         | `README.md` glossary · station says "two strings, not one truncated"; `ux/wireframes.md` § Fresh mount says "the label's first word" — **opposite mechanisms**; `display-labels.ts` carries one string per phase | **OPEN — round 6 I4.** Decides a `Station` field and `display-labels.ts`'s value shape; free now, a codemod after 0.3 |
