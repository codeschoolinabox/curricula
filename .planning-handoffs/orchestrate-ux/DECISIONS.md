...
| C12 | **The caption** — the one line beneath the rail: its total precedence (cause line › count line › nothing) and, until 2026-08-18, its missing NAME | `README.md` glossary · the caption, glossary · display labels | `DOCS.md` § The render projection carries the precedence and the noun; `ux/wireframes.md` § Fresh mount, § The parse breaks, § The spelling breaks (drawn)                                                                                                                                                                                                                                    | **SETTLED IN FULL** — I7's half at `b086afe6`, M12's half at `964ec315` (the object is named `the caption`). **The row itself was MALFORMED from `b086afe6` until 2026-08-18** — 7 cells against a 5-cell header, two contradictory statuses, and a residual fragment of an earlier revision. No linter sees a pipe-count defect (round 10 IMPORTANT 6) |
...
| **Three package-scope reconciliations**             | H8                         | `overlay` and `verdict` are narrowed against the package glossary, and **`house token` is a `--sl-` namespace this region mints for the whole package** while its own entry admits it cannot rule for the others ("a migration with named files, **owed to whichever campaign claims it**"). All three divergences are DECLARED in this region's glossary; the upward move edits the package README, which this campaign has no authority over. **Recorded so the boundary question does not die when this campaign prunes** (round 9 MINOR 7, MINOR 8).                                                                                                                                                                                                |
