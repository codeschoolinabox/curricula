...
async function p(label, code) {
	try {
		const s = await record(code, CFG);
		console.log(`${label}: OK (${s.length} steps)`);
	} catch (e) {
...
await p(
	'S3 minimal getter, no writes',
	'const o = { get g() { return 7; } }; o;',
);
await p(
	'S4 getter object never referenced after decl',
	'const o = { get g() { return 7; } };',
);
