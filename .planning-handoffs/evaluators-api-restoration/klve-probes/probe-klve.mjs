...
// P1 — output init step number
{
	...
	if (steps)
		console.log(
			'  first step JSON:',
			JSON.stringify({ step: steps[0].step, category: steps[0].category }),
		);
}
...
{
	const steps = await probe(
		'P2b dead code after return',
		'function f() { return 7; console.log("DEAD"); } f();',
		CFG(),
	);
	if (steps) {
...
// P4 — s++ on a string: stored + reported values
{
	...
	if (steps) {
		...
		console.log(
			'  final s value:',
			JSON.stringify(finalRead?.value),
			typeof finalRead?.value,
		);
	}
}
...
// P14 — top-level `this` and global reach inside the "sandbox"
{
	const steps = await probe(
		'P14 global reach',
		'const g = (typeof globalThis !== "undefined"); g;',
		CFG(),
	);
	if (steps) {
		const finalRead = steps
