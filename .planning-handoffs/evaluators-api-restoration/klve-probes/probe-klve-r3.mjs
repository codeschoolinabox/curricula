...
try {
	...
	console.log(
		`T1 export line3: ${e?.constructor?.name}: ${e?.message} | loc=${safe(e?.loc)}`,
	);
}
...
try {
	...
	console.log(
		`T1b parse-error control: ${e?.constructor?.name}: ${e?.message} | loc=${safe(e?.loc)}`,
	);
}
...
try {
	...
	console.log(
		`T1c import line3: ${e?.constructor?.name}: ${e?.message} | loc=${safe(e?.loc)}`,
	);
}
...
{
	const code =
		'class Foo { constructor() { this.k = 1; } } const f = new Foo(); f;';
	const s1 = await record(code, CFG);
	const s2 = await record(code, CFG);
	const v1 = s1
		.filter((s) => s.type === 'Identifier' && s.time === 'after')
		.at(-1)?.value;
	const v2 = s2
		.filter((s) => s.type === 'Identifier' && s.time === 'after')
		.at(-1)?.value;
	console.log(
		'T2 cross-call fake ctor: ctor names',
		v1?.constructor?.name,
		v2?.constructor?.name,
		'| identical constructor object:',
...
// T3 — non-enumerable and symbol-keyed properties invisible in described values
{
	const steps = await record(
		'const o = { vis: 1 }; Object.defineProperty(o, "hid", { value: 2 }); o[Symbol("s")] = 3; o;',
		CFG,
	);
	const v = steps
		.filter((s) => s.type === 'Identifier' && s.time === 'after')
		.at(-1)?.value;
	console.log(
		'T3 snapshot boundary: keys =',
		safe(Object.keys(v ?? {})),
		'| hid in v:',
		v ? 'hid' in v : null,
		'| symbol keys:',
		v ? Object.getOwnPropertySymbols(v).length : null,
	);
}
