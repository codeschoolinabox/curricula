...
// N10 — getter side effects: how many times does the getter fire under tracing?
{
	...
	if (steps) {
		...
		console.log(
			'  n after tracing:',
			safe(last?.value),
			'   [natively: 0 — o is never read via .g]',
		);
	}
}
...
// N12 — async function: are post-await steps traced?
{
	...
	if (steps) {
		const assigns = steps.filter(
			(s) => s.type === 'AssignmentExpression' && s.time === 'after',
		);
		console.log(
			'  assignment steps seen:',
			assigns.map((s) => safe(s.value)).join(', '),
			'  [mark=2 step present?]',
		);
		const last = steps
			.filter((s) => s.type === 'Identifier' && s.time === 'after')
...
{
	await probe('N4 logs key on quiet steps', 'let x = 1;', CFG(), () => true);
}
